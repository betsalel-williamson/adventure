...
import { classifyRoomFingerprint, describeMotionGridDelta, extractLocationLineForFingerprint, fingerprintLocationFromGameOutput, sessionLocationFingerprintFromGameOutput, canonicalExplorationFingerprint, fsmLabelFromGetinCommand, graphEdgeLabelFromCommand, InferredExplorationMap, detectLocationStagnation, fingerprintLocationFromExcerpt, MOTION_GRID_DELTA, canonicalMotionPrimaryForDedup, cardinalRotationForCellKey, orderedEscapePrimariesForCellKey, vecKey, } from "./inferredExplorationMap.js";
...
describe("fingerprintLocationFromExcerpt", () => {
    it("normalizes whitespace and caps length", () => {
        expect(fingerprintLocationFromExcerpt("  YOU ARE HERE  ")).toBe("YOU ARE HERE");
    });
});
    ...
    it("omits diagonal compass primaries from the tail unless ADVENTURE_LLM_DIAGONAL_COMPASS_MOTION is set", () => {
        ...
        try {
            ...
            if (prev === undefined)
                ...
                process.env.ADVENTURE_LLM_DIAGONAL_COMPASS_MOTION = prev;
        }
    });
});
describe("graphEdgeLabelFromCommand", () => {
    it("annotates noun-as-primary GETIN lines for graph readers", () => {
        ...
        expect(graphEdgeLabelFromCommand(keysKey)).toBe("KEYS KEY (use TAKE/GET + object)");
    });
...
describe("extractLocationLineForFingerprint", () => {
    it("prefers YOU ARE after a blocked-move prefix on one line", () => {
        const text = "THERE IS NO WAY TO GO THAT DIRECTION. YOU ARE IN A MAZE OF TWISTY LITTLE PASSAGES, ALL ALIKE.";
        expect(extractLocationLineForFingerprint(text)).toContain("YOU ARE IN A MAZE");
    });
    ...
    it("skips YOU ARE ALREADY CARRYING (not a place line)", () => {
        expect(extractLocationLineForFingerprint("YOU ARE ALREADY CARRYING IT!")).toBe(null);
    });
    it("skips carrying line and uses the next YOU ARE place line", () => {
        const text = "YOU ARE ALREADY CARRYING IT. YOU ARE INSIDE A BUILDING FOR A LARGE SPRING.";
...
describe("fingerprintLocationFromGameOutput", () => {
    it("fingerprints Colossal Cave stream slit room (no YOU ARE line)", () => {
        const text = "AT YOUR FEET ALL THE WATER OF THE STREAM SPLASHES INTO A 2 INCH SLIT IN THE ROCK. DOWNSTREAM THE STREAMBED IS BARE ROCK.";
    ...
    it("does not fingerprint carrying-only messages as a room", () => {
        expect(fingerprintLocationFromGameOutput("YOU ARE ALREADY CARRYING IT!")).toBe("");
    });
...
describe("canonicalExplorationFingerprint", () => {
    it("merges outdoor road-start wording and OUT return line to one fingerprint", () => {
        const a = fingerprintLocationFromExcerpt("YOU ARE STANDING AT THE END OF A ROAD BEFORE A SMALL BRICK");
        const b = fingerprintLocationFromExcerpt("YOU ARE STANDING AT THE END OF A ROAD BEFORE A SMALL BRICK BUILDING.");
        const c = fingerprintLocationFromExcerpt("YOU'RE AT END OF ROAD AGAIN.");
        expect(canonicalExplorationFingerprint(a)).toBe(canonicalExplorationFingerprint(b));
        expect(canonicalExplorationFingerprint(c)).toBe(canonicalExplorationFingerprint(b));
    });
    it("does not merge hill-in-road (LTEXT 2)", () => {
        const hill = fingerprintLocationFromExcerpt("YOU'RE AT HILL IN ROAD.");
        const road = fingerprintLocationFromExcerpt("YOU ARE STANDING AT THE END OF A ROAD BEFORE A SMALL BRICK BUILDING.");
        expect(canonicalExplorationFingerprint(hill)).not.toBe(canonicalExplorationFingerprint(road));
    });
});
describe("sessionLocationFingerprintFromGameOutput", () => {
    it("uses loose place prose before staling to the previous room fingerprint", () => {
        const prev = "YOU ARE IN A VALLEY IN THE FOREST BESIDE A STREAM TUMBLING ALONG A ROCKY BED.";
        const prevFp = fingerprintLocationFromExcerpt(prev);
        const atSlit = "AT YOUR FEET ALL THE WATER OF THE STREAM SPLASHES INTO A 2 INCH SLIT IN THE ROCK. DOWNSTREAM THE STREAMBED IS BARE ROCK.";
        expect(sessionLocationFingerprintFromGameOutput(atSlit, prevFp)).not.toBe(prevFp);
    });
    it("falls back to previous fingerprint for OK-only output", () => {
        const prev = "YOU ARE IN A VALLEY.";
        expect(sessionLocationFingerprintFromGameOutput("OK", prev)).toBe(fingerprintLocationFromExcerpt(prev));
    });
});
...
describe("classifyRoomFingerprint", () => {
    it("labels road before nearby building word", () => {
        const { roomKind } = classifyRoomFingerprint("YOU ARE STANDING AT THE END OF A ROAD BEFORE A SMALL BRICK BUILDING.");
        expect(roomKind).toBe("road");
    });
    it("labels well house as building", () => {
        const { roomKind } = classifyRoomFingerprint("YOU ARE INSIDE A BUILDING, A WELL HOUSE FOR A LARGE SPRING.");
        expect(roomKind).toBe("building");
    });
    it("labels maze", () => {
        const { roomKind } = classifyRoomFingerprint("YOU ARE IN A MAZE OF TWISTY LITTLE PASSAGES, ALL ALIKE.");
...
describe("detectLocationStagnation", () => {
    it("is false until enough successful same-fp turns", () => {
        ...
        expect(detectLocationStagnation([
...
describe("InferredExplorationMap", () => {
    it("records SOUTH from valley to AT YOUR FEET stream slit as a new cell", () => {
        ...
        m.seedFromTranscript("YOU ARE IN A VALLEY IN THE FOREST BESIDE A STREAM TUMBLING ALONG A ROCKY BED.\n");
        const south = interpretedToGetinLine({ primaryToken: "SOUTH" });
        const slitText = "AT YOUR FEET ALL THE WATER OF THE STREAM SPLASHES INTO A 2 INCH SLIT IN THE ROCK. DOWNSTREAM THE STREAMBED IS BARE ROCK.";
        m.recordOutcome(south, slitText, false);
        const snap = m.toSnapshot();
        expect(snap.cells.length).toBe(2);
        const southEdges = snap.directedEdges.filter((e) => e.kind === "move" && e.label === "SOUTH");
    ...
    it("deduplicates repeated move edges between the same nodes", () => {
        ...
        m.recordOutcome("NORTH", fpB, false);
        const snap = m.toSnapshot();
        const northMoves = snap.directedEdges.filter((e) => e.kind === "move" && e.label === "NORTH");
    ...
    it("treats N and NORTH as one edge for deduplication", () => {
        ...
        m.recordOutcome("NORTH", fpB, false);
        const snap = m.toSnapshot();
        const northMoves = snap.directedEdges.filter((e) => e.kind === "move" && e.label.startsWith("N"));
        expect(northMoves.length).toBe(1);
    });
    ...
    it("applies dz -1 for IN into a new room (grate → chamber beneath)", () => {
        const m = new InferredExplorationMap();
        m.seedFromTranscript("YOU ARE IN A 20 FOOT DEPRESSION FLOORED WITH BARE DIRT. SET INTO THE DIRT IS A STRONG STEEL GRATE MOUNTED IN CONCRETE.\n");
        expect(m.getCurrentVec().z).toBe(0);
        const inn = interpretedToGetinLine({ primaryToken: "IN" });
        m.recordOutcome(inn, "YOU ARE IN A SMALL CHAMBER BENEATH A 3X3 STEEL GRATE TO THE SURFACE. A LOW CRAWL OVER COBBLES LEADS INWARD TO THE WEST. THE GRATE IS LOCKED.\n", false);
    ...
    it("applies dz +1 for OUT back to a higher level", () => {
        ...
        m.seedFromTranscript("YOU ARE IN A 20 FOOT DEPRESSION FLOORED WITH BARE DIRT. SET INTO THE DIRT IS A STRONG STEEL GRATE MOUNTED IN CONCRETE.\n");
        const inn = interpretedToGetinLine({ primaryToken: "IN" });
        m.recordOutcome(inn, "YOU ARE IN A SMALL CHAMBER BENEATH A 3X3 STEEL GRATE TO THE SURFACE.\n", false);
        expect(m.getCurrentVec().z).toBe(-1);
        const out = interpretedToGetinLine({ primaryToken: "OUT" });
        m.recordOutcome(out, "YOU ARE IN A 20 FOOT DEPRESSION FLOORED WITH BARE DIRT. SET INTO THE DIRT IS A STRONG STEEL GRATE MOUNTED IN CONCRETE.\n", false);
        expect(m.getCurrentVec().z).toBe(0);
    });
    it("OUT from well house snaps to occupied surface cell when location text differs from seed", () => {
        const m = new InferredExplorationMap();
        m.seedFromTranscript("YOU ARE STANDING AT THE END OF A ROAD BEFORE A SMALL BRICK BUILDING.\n");
        expect(m.getCurrentVec()).toEqual({ x: 0, y: 0, z: 0 });
        const inn = interpretedToGetinLine({ primaryToken: "IN" });
        m.recordOutcome(inn, "YOU ARE INSIDE A BUILDING, A WELL HOUSE FOR A LARGE SPRING.\n", false);
        expect(m.getCurrentVec().z).toBe(-1);
        const out = interpretedToGetinLine({ primaryToken: "OUT" });
    ...
    it("does not split end-of-road into two cells when the first sentence ends after SMALL BRICK", () => {
        ...
        m.seedFromTranscript("YOU ARE STANDING AT THE END OF A ROAD BEFORE A SMALL BRICK BUILDING.\n");
        const look = interpretedToGetinLine({ primaryToken: "LOOK" });
        m.recordOutcome(look, "YOU ARE STANDING AT THE END OF A ROAD BEFORE A SMALL BRICK. AROUND YOU IS A FOREST.\n", false);
    ...
    it("toSnapshot lists cells, current position, and exit outcomes", () => {
        ...
        expect(cell0?.some((e) => e.primary === "NORTH" && e.outcome === "moved")).toBe(true);
    ...
    it("blocked move does not change position", () => {
        ...
        const south = interpretedToGetinLine({ primaryToken: "SOUTH" });
        m.recordOutcome(south, `THERE IS NO WAY TO GO THAT DIRECTION. ${mazeLine}`, false, true);
        expect(m.getCurrentVec()).toEqual(before);
        expect(m.getExitOutcome(vecKey(before), "SOUTH")).toBe("same");
    });
});
//# sourceMappingURL=inferredExplorationMap.test.js.map