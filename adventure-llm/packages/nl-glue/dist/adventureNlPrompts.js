...
export function resolveInterpretPromptExamples(providerId) {
    if (resolveFastInterpretPrompt())
        return false;
    const v = process.env.ADVENTURE_LLM_INTERPRET_PROMPT_EXAMPLES?.trim().toLowerCase();
    if (v === "1" || v === "true" || v === "yes")
        return true;
    if (v === "0" || v === "false" || v === "no")
        return false;
 ...
 */
export const ADVENTURE_LLM_INTERPRET_COMPACT_HELP_HINT = "If the user asks for instructions, hints, or how to play, use primaryToken HELP and omit secondaryToken.";
...
export const ADVENTURE_LLM_INTERPRET_ROLE = "You are mapping user input to Colossal Cave Adventure parser tokens (max 5 letters each, like the original game).";
/** Shorter role for small local models (MLX): less preamble, same task. */
export const ADVENTURE_LLM_INTERPRET_ROLE_COMPACT = "Map the user's line to Colossal Cave Adventure parser tokens (max 5 letters each). Reply with JSON only.";
/**
...
export const ADVENTURE_LLM_AUTOPLAY_LOOT_FUNNEL_SYSTEM_SLM = `**Loot gate (this turn):** **Items** lists ground objects you are not yet carrying. You MUST output {"primaryToken":"TAKE","secondaryToken":"<item>"} (or GET, same shape) for one of those objects. Do not emit travel-only JSON until **Items** is (none visible). **Cand_Move** is hidden — use **Cand_Act**/**Cand_Obj** only.`;
/** Autoplay: role for the planner + session-memory context (same rules as interpret). */
export const ADVENTURE_LLM_AUTOPLAY_PLANNER_ROLE = "You are playing Colossal Cave Adventure as the adventurer. Choose the next parser command to continue. Reply ONLY with JSON; the exact key list is specified again at the end of the full prompt after this context.";
export const ADVENTURE_LLM_AUTOPLAY_PLANNER_ROLE_COMPACT = "You are the adventurer. Choose the next parser command. Reply ONLY with JSON; keys are repeated at the end of the prompt.";
/**
...
export function resolveAutoplayPromptMode() {
    const v = process.env.ADVENTURE_LLM_AUTOPLAY_PROMPT_MODE?.trim().toLowerCase();
    if (v === "full")
        return "full";
    return "explore";
}
...
Use only primaryToken + secondaryToken when needed. Shorter JSON is better.`;
    ...
    const rules = ADVENTURE_LLM_PARSER_TOKEN_RULES;
