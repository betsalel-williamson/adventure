...
import { buildVocabHint } from "@adventure-llm/nl-glue";
...
describe("buildVocabHint", () => {
    ...
    beforeEach(() => {
        process.env.ADVENTURE_LLM_VOCAB_CATEGORIES_FILE = path.join(tmpdir(), `no-ai-vocab-${Date.now()}.json`);
    ...
    it("grouped compact uses shorter blurbs", () => {
        ...
        expect(g).toMatch(/\*\*Motion/i);
        expect(g.length).toBeLessThan(buildVocabHint(db, 30, {
            grouped: true,
            structuredGroups: false,
            compact: false,
        }).length);
...
//# sourceMappingURL=vocabHint.test.js.map