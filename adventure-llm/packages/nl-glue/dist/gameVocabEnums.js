 ...
 */
export function vocabTokensForLlmEnums(db) {
    const set = new Set(collectGameVocabTokens(db));
    for (const t of SYNTHETIC_ENUM_TOKENS)
        set.add(t);
    if (!isDiagonalCompassMotionEnabled()) {
        for (const d of DIAGONAL_COMPASS_MOTION_TOKEN_SET)
            set.delete(d);
    }
 ...
 */
export function openAiInterpretCommandJsonSchema(tokenEnum) {
    const e = [...tokenEnum];
    return {
        type: "object",
        properties: {
 ...
 */
export function openAiAutoplayPlannerJsonSchema(tokenEnum) {
    const e = [...tokenEnum];
    return {
        type: "object",
        properties: {
...
//# sourceMappingURL=gameVocabEnums.js.map