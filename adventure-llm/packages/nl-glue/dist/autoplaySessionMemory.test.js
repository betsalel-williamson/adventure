...
import { AutoplaySessionMemory, buildSituationalCandidateTokens, detectAlternatingLocationCommandLoop, fingerprintLocationFromGameOutput, formatSituationalCandidatesSection, gameOutputLooksLikeBlockedMove, gameOutputLooksLikeParserRejection, gameOutputLooksLikePlayAgainPrompt, interpretedToGetinLine, listVisibleAdventureObjectsInText, normalizeGetinLineKey, } from "@adventure-llm/nl-glue";
...
describe("take failure object learning", () => {
    const dbPath = path.join(fileURLToPath(new URL(".", import.meta.url)), "../../../../adventure.dat");
    it("records failed TAKE/GET and exposes ATAB word via getRoomTakeFailureObjectAtabWords", () => {
        ...
        m.recordCommandOutcome(line, "I DON'T KNOW HOW TO APPLY THAT WORD HERE.\n", { adventureDb: db });
    ...
    it("learns YOU CAN'T style refusal as take failure", () => {
        ...
        m.recordCommandOutcome(interpretedToGetinLine({ primaryToken: "GET", secondaryToken: "GRATE" }), "YOU CAN'T GET THAT.\n", { adventureDb: db });
        expect(m.getRoomTakeFailureObjectAtabWords()).toContain("GRATE");
    });
...
describe("gameOutputLooksLikeParserRejection", () => {
    it("detects apply-word rejection", () => {
        expect(gameOutputLooksLikeParserRejection("I DON'T KNOW HOW TO APPLY THAT WORD HERE.")).toBe(true);
    });
    ...
    it("is false for normal room text", () => {
        expect(gameOutputLooksLikeParserRejection("YOU ARE INSIDE A BUILDING. THERE ARE KEYS HERE.")).toBe(false);
    });
});
describe("gameOutputLooksLikeBlockedMove", () => {
    it("detects no way to go that direction", () => {
        expect(gameOutputLooksLikeBlockedMove("THERE IS NO WAY TO GO THAT DIRECTION.\n\nYOU ARE IN A MAZE OF TWISTY LITTLE PASSAGES, ALL ALIKE.\n")).toBe(true);
    });
    it("is false for successful move text", () => {
        expect(gameOutputLooksLikeBlockedMove("YOU ARE IN A MAZE OF TWISTY LITTLE PASSAGES, ALL ALIKE.\n")).toBe(false);
    });
});
describe("gameOutputLooksLikePlayAgainPrompt", () => {
    it("detects game over and play again", () => {
        expect(gameOutputLooksLikePlayAgainPrompt("IT GETS YOU!\n\nGAME IS OVER.  PLAY AGAIN?\n")).toBe(true);
    });
    it("is false for normal play", () => {
        expect(gameOutputLooksLikePlayAgainPrompt("YOU ARE IN OPEN FOREST.\n")).toBe(false);
    });
});
describe("AutoplaySessionMemory", () => {
    it("updates location hint from the latest room line (YOU'RE, not stale opening)", () => {
        const m = new AutoplaySessionMemory();
        m.seedOpening("YOU ARE STANDING AT THE END OF A ROAD BEFORE A SMALL BRICK BUILDING.\n");
    ...
    it("seeds opening and records turns", () => {
        ...
        expect(m.getRecentRawTail()).toMatch(/> EAST[\s\S]*YOU ARE INSIDE A WELL HOUSE/);
    });
    it("buildInteractiveInterpretPrefix includes session and candidates", () => {
        const db = loadDatFile(datPath);
        const m = new AutoplaySessionMemory();
        m.seedOpening("YOU ARE STANDING AT THE END OF A ROAD BEFORE A SMALL BUILDING.\n");
    ...
    it("does not treat YOU ARE ALREADY CARRYING as location hint", () => {
        const m = new AutoplaySessionMemory();
        m.seedOpening("YOU'RE IN FOREST.\n> TAKE KEYS\nYOU ARE ALREADY CARRYING IT!\n");
    ...
    it("adds parser rejection section when last outcome was rejected", () => {
        ...
        m.recordCommandOutcome("BUILD   ", "I DON'T KNOW HOW TO APPLY THAT WORD HERE.\nYOU ARE STILL HERE.\n");
    ...
    it("MLX structured compact: system has SLM Rules+Constraints; user has Loc+Cand only token list (no DOT)", () => {
        ...
        try {
            ...
            const parts = m.buildPlannerMxStructuredPrompt(20_000, {
                compact: true,
                situationalSection: "## Situation candidates (heuristic)\nEAST, WEST, TAKE",
            });
            ...
            expect(parts.user.indexOf("### SESSION")).toBeLessThan(parts.user.indexOf("**Loc:**"));
            expect(parts.user.indexOf("**Loc:**")).toBeLessThan(parts.user.indexOf("**Cand:**"));
            ...
            if (prev === undefined)
                ...
                process.env.ADVENTURE_LLM_AUTOPLAY_PROMPT_MODE = prev;
        }
    });
    ...
    it("structured dashboard puts ADVENTURE STATE and TASK before SPATIAL HINT", () => {
        ...
        expect(p.indexOf("### ADVENTURE STATE")).toBeLessThan(p.indexOf("### SPATIAL HINT (session, text)"));
        expect(p.indexOf("### TASK")).toBeLessThan(p.indexOf("### SPATIAL HINT (session, text)"));
    });
    it("lists parser-rejected GETIN lines and clears after success", () => {
        const m = new AutoplaySessionMemory();
        m.seedOpening("START\n");
        const buildLine = interpretedToGetinLine({ primaryToken: "BUILD" });
        m.recordCommandOutcome(buildLine, "I DON'T KNOW HOW TO APPLY THAT WORD HERE.\n");
        let p = m.buildPlannerUserPrompt(20_000, "BUILD");
        expect(p).toContain("Parser-rejected commands");
        expect(p).toContain("`BUILD`");
        m.recordCommandOutcome(interpretedToGetinLine({ primaryToken: "LOOK" }), "YOU SEE A LAMP.\n");
    ...
    it("avoidRedundantTakeWhenCarrying substitutes when already carrying object", () => {
        ...
        expect(m.getStructuredInventory().join(" ").toUpperCase()).toContain("KEYS");
    ...
    it("MLX loot funnel moves **Items**/**Task** after Cand and omits Break-* streak in Task", () => {
        const db = loadDatFile(datPath);
        const m = new AutoplaySessionMemory();
        m.seedOpening("YOU ARE INSIDE A BUILDING. THERE ARE SOME KEYS ON THE GROUND.\n", { adventureDb: db });
        const west = interpretedToGetinLine({ primaryToken: "WEST" });
        const room = "YOU ARE STILL INSIDE. A SHINY BRASS LAMP IS HERE. KEYS ON THE GROUND.\n";
        m.recordCommandOutcome(west, room, { adventureDb: db });
        m.recordCommandOutcome(west, room, { adventureDb: db });
        m.recordCommandOutcome(west, room, { adventureDb: db });
        const situational = formatSituationalCandidatesSection(db, buildSituationalCandidateTokens(db, m.getRecentRawTail(), {
            lootFunnel: true,
            exploreFirst: true,
        }), undefined, { slmGrouped: true, lootFunnelDeferCandMove: true });
        const parts = m.buildPlannerMxStructuredPrompt(20_000, {
            compact: true,
    ...
    it("getStructuredInventory detects OK after many lines of output", () => {
        ...
        m.recordCommandOutcome(interpretedToGetinLine({ primaryToken: "TAKE", secondaryToken: "LAMP" }), `${prefix}\nOK\n`);
    ...
    it("getStructuredInventory clears inferred items after Fortran INIT DONE (play again)", () => {
        ...
        m.recordCommandOutcome(y, "INIT DONE\n\nYOU ARE STANDING AT THE END OF A ROAD BEFORE A SMALL BRICK\nBUILDING.\n");
        expect(m.getStructuredInventory().length).toBe(0);
...
//# sourceMappingURL=autoplaySessionMemory.test.js.map