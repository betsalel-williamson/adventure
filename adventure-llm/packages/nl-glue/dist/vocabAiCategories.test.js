...
import { buildVocabHint, collectGameVocabTokens, formatAiGroupedVocabularyHint, loadAiVocabCategoriesForHint, pickWordsFromAiGroups, resolveAiVocabCategoriesPath, } from "@adventure-llm/nl-glue";
...
describe("buildVocabHint with AI file", () => {
    ...
    beforeEach(() => {
        tmpDir = mkdtempSync(path.join(tmpdir(), "vocab-ai-"));
        process.env = { ...prev };
        process.env.ADVENTURE_LLM_VOCAB_CATEGORIES_FILE = path.join(tmpDir, "vc.json");
        const all = collectGameVocabTokens(db);
        const mid = Math.ceil(all.length / 2);
        writeFileSync(process.env.ADVENTURE_LLM_VOCAB_CATEGORIES_FILE, JSON.stringify({
            version: 1,
            groups: [
                {
                    id: "g1",
...
//# sourceMappingURL=vocabAiCategories.test.js.map