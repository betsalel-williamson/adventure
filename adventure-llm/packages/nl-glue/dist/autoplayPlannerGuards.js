...
export function planAfterAutoplayGuards(memory, plan, log) {
    ...
    if (interpretedToGetinLine(toInterpreted(plan)) !==
        ...
        log("adventure-llm: autoplay — replaced plan that repeated a parser-rejected GETIN line\n");
    }
    ...
    if (interpretedToGetinLine(toInterpreted(beforeTakeCarry)) !==
        interpretedToGetinLine(toInterpreted(planSafe))) {
        log("adventure-llm: autoplay — replaced plan: TAKE/GET object already in inventory\n");
    }
    ...
    if (interpretedToGetinLine(toInterpreted(beforeOsc)) !==
        interpretedToGetinLine(toInterpreted(planSafe))) {
        log("adventure-llm: autoplay — replaced plan that would continue a two-location loop\n");
    }
    ...
    if (interpretedToGetinLine(toInterpreted(beforeStag)) !==
        interpretedToGetinLine(toInterpreted(planSafe))) {
        log("adventure-llm: autoplay — replaced plan due to location stagnation (inferred map)\n");
    }
    ...
    if (interpretedToGetinLine(toInterpreted(beforeLook)) !==
        interpretedToGetinLine(toInterpreted(planSafe))) {
        log("adventure-llm: autoplay — replaced plan: LOOK/EXAMI already used without leaving this room (inferred map)\n");
    }
    return planSafe;
}
