...
export function buildVocabAiCategorizationPrompt(
...
export async function generateAiVocabCategoriesWithLlm(
  ...
  if (!parsed.success) {
    throw new Error(
      `vocab AI categorization: invalid JSON from model: ${parsed.error.message}`,
    );
  }
  ...
  if (missing.length > 0) {
    filtered.push({
      ...
      blurb:
        "Automatically added: tokens the model omitted but the game table includes.",
      tokens: missing,
    });
  }
