...
Install deps: from adventure-lm directory run ``uv venv`` then ``uv sync`` (uses ``pyproject.toml``).
...
Default model (override with ADVENTURE_LM_MLX_MODEL): mlx-community/gemma-2-2b-it
"""
...
def _default_mlx_temp() -> float:
    v = os.environ.get("ADVENTURE_LM_MLX_TEMP", "0.75").strip()
    try:
        t = float(v)
...
def _default_stop_strings() -> list[str]:
    raw = os.environ.get("ADVENTURE_LM_MLX_STOP", "").strip()
    if not raw:
        return []
...
def main() -> None:
    model_id = os.environ.get(
        "ADVENTURE_LM_MLX_MODEL", "mlx-community/gemma-2-2b-it"
    ).strip()
    ...
    print(f"adventure-lm mlx: loading {model_id}…", file=sys.stderr, flush=True)
    model, tokenizer = load(model_id)
    print("adventure-lm mlx: model loaded.", file=sys.stderr, flush=True)
