...
import { resolveAutoplayPromptMode } from "@adventure-lm/lm-glue";
import {
...
import type { PlannerUserPromptInput } from "@adventure-lm/lm-glue";
import {
...
function resolveWebPort(): number {
  const v = process.env.ADVENTURE_LM_WEB_PORT?.trim();
...
export function createAutoplayDashboardServer(
  ...
  if (pool !== null) {
    process.stderr.write(
      `adventure-lm: dashboard text model pool capacity: ${resolveWebMaxLoadedModels()} (ADVENTURE_LM_WEB_MAX_LOADED_MODELS)\n`,
    );
  }
  ...
  function createDashboardSession(sessionId: string): DashboardSession {
    ...
    const flushBenchmarkRun = (
      ...
      const eventId =
        process.env.ADVENTURE_LM_BENCHMARK_EVENT_ID?.trim() || null;
      ...
      const dbg = process.env.ADVENTURE_LM_DEBUG?.trim();
    ...
    if (
      ...
      if (bdb === null) {
        jsonResponseWithSessionCookie(
          res,
          503,
          { error: "Benchmark runs disabled (ADVENTURE_LM_BENCHMARK_RUNS=0)" },
          cookieOpts(sess, newSession),
        );
        return;
      }
    ...
    if (pathname === "/events" && req.method === "GET") {
      ...
      if (!pool || !textLlmConfigured) {
        sseWrite(res, "session_error", {
          message:
            "No text model configured (GEMINI_API_KEY, ADVENTURE_LM_HTTP_MODEL, or MLX). See adventure-lm/.env.example.",
        });
        return;
      }
          ...
          try {
            ...
            if (
              ...
              sref.broadcast("log_line", {
                line: "adventure-lm: text model unavailable (quota, rate limit, network, or service error).\n",
                step: 0,
                channel: "planner",
              });
            }
...
async function main(): Promise<void> {
  ...
  if (dbgOn) {
    if (process.env.ADVENTURE_LM_DEBUG_LOG?.trim()) {
      ...
      process.stderr.write(
        `adventure-lm: interaction logs → ${path.join(process.cwd(), ".cache", "llm-sessions", "<sessionId>.jsonl")} (web) or ${path.join(process.cwd(), ".cache", "llm-interactions.jsonl")} (CLI)\n`,
      );
    }
  }
  ...
  if (!insecure && tls === null) {
    process.stderr.write(
      "adventure-lm: No TLS certificate found. Run: npm run web:tls-init\n" +
        "  Or set ADVENTURE_LM_WEB_TLS_KEY / ADVENTURE_LM_WEB_TLS_CERT, or use ADVENTURE_LM_WEB_INSECURE_HTTP=1 (not recommended).\n",
    );
    process.exit(1);
  }
  if (insecure) {
    process.stderr.write(
      "adventure-lm: ADVENTURE_LM_WEB_INSECURE_HTTP=1 — session cookies are not Secure; use only on trusted networks.\n",
    );
  }
  ...
  server.listen(port, "127.0.0.1", () => {
    process.stderr.write(
      `adventure-lm: autoplay web dashboard → ${proto}://127.0.0.1:${port}/\n`,
    );
    process.stderr.write(
