 ...
 * logging (see ADVENTURE_LM_DEBUG* and ADVENTURE_LM_CACHE_DIR in .env.example).
 * Pass --autoplay for self-acting mode (model-driven moves; see ADVENTURE_LM_AUTOPLAY_* in .env.example).
...
import { AutoplaySessionMemory } from "@adventure-lm/lm-glue";
import {
  interpretWithTextLlm,
  resolveTextLlmFromEnv,
} from "../nl/adventureTextLlm.js";
import { resolveInterpretPromptBuildOptions } from "@adventure-lm/lm-glue";
...
import { instructionIntentToHelpCommand } from "@adventure-lm/lm-glue";
import {
  interpretedToGetinLine,
  swapInterpretedTokens,
} from "@adventure-lm/lm-glue";
import type { TextLlm } from "@adventure-lm/lm-glue";
...
/** dist/cli -> adventure-lm -> repo root */
...
function printClassicBanner(
  ...
  process.stderr.write(
    "adventure-lm — classic mode uses the original Fortran Colossal Cave engine.\n",
  );
  if (!hasTextLlmConfigured) {
    process.stderr.write(
      "Natural language needs a text model: set GEMINI_API_KEY and/or ADVENTURE_LM_HTTP_* (see adventure-lm/.env.example). Omit --classic once configured.\n",
    );
...
async function ensureMlxWorkerReady(client: TextLlm): Promise<void> {
  ...
  process.stderr.write("adventure-lm: loading MLX model (one-time)…\n");
  await client.preloadWorker();
}

/** Default on: heuristic session block + situational tokens for interactive NL (set ADVENTURE_LM_INTERACTIVE_SESSION=0 to disable). */
function resolveInteractiveSessionMemoryEnabled(): boolean {
  const v = process.env.ADVENTURE_LM_INTERACTIVE_SESSION?.trim().toLowerCase();
...
async function main(): Promise<void> {
  ...
  if (args.includes("--debug")) {
    process.env.ADVENTURE_LM_DEBUG ??= "1";
  }
  ...
  if (autoplay && !hasTextLlm) {
    process.stderr.write(
      "adventure-lm: --autoplay requires a text model (GEMINI_API_KEY and/or ADVENTURE_LM_HTTP_MODEL; see .env.example).\n",
    );
    process.exitCode = 1;
    return;
  }
  if (autoplay && forceClassic) {
    process.stderr.write(
      "adventure-lm: --autoplay cannot be used with --classic.\n",
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (autoplay && textLlm) {
    ...
    try {
      ...
      if (shouldFallbackToClassicForLlmError(err, textLlm.providerId)) {
        process.stderr.write(
          "\nadventure-lm: text model unavailable (quota, rate limit, network, or service error). Autoplay stopped.\n",
        );
        process.exitCode = 1;
        return;
      }
  ...
  process.stderr.write(
    "adventure-lm: language-model-assisted input enabled.\n",
  );
  process.stderr.write(
    "adventure-lm: type naturally at > ; input is translated to game vocabulary. .quit or :q ends the session.\n",
  );
  ...
  if (cacheDir) {
    process.stderr.write(`adventure-lm: response cache → ${cacheDir}\n`);
  }
  ...
  try {
    ...
    if (shouldFallbackToClassicForLlmError(err, textLlm!.providerId)) {
      process.stderr.write(
        "\nadventure-lm: text model is unavailable (quota, rate limit, or service error). Switching to classic mode: type parser words directly (e.g. EAST, TAKE LAMP).\n\n",
      );
      printClassicBanner(hasTextLlm, forceClassic);
      runClassicInteractive();
      return;
