...
describe("createAutoplayDashboardServer", () => {
  afterEach(() => {
    delete process.env.ADVENTURE_LM_PROMPT_PROJECTS_DIR;
    delete process.env.ADVENTURE_LM_BENCHMARK_DB;
    delete process.env.ADVENTURE_LM_BENCHMARK_RUNS;
    delete process.env.ADVENTURE_LM_SUBSYSTEM_REPLICA_DIR;
    resetBenchmarkRunsDbSingleton();
  ...
  it("POST /api/subsystem-sync applies a linear batch (ADR0008)", async () => {
    ...
    process.env.ADVENTURE_LM_SUBSYSTEM_REPLICA_DIR = replicaDir;
  ...
  it("GET /api/session includes browserOrchestratedAutoplay", async () => {
    const prev = process.env.ADVENTURE_LM_BROWSER_ORCHESTRATED_AUTOPLAY;
    process.env.ADVENTURE_LM_BROWSER_ORCHESTRATED_AUTOPLAY = "1";
    const server = testHttpDashboard();
    await new Promise<void>((resolve) => {
      server.listen(0, "127.0.0.1", () => resolve());
    });
    ...
    if (prev === undefined)
      delete process.env.ADVENTURE_LM_BROWSER_ORCHESTRATED_AUTOPLAY;
    else process.env.ADVENTURE_LM_BROWSER_ORCHESTRATED_AUTOPLAY = prev;
  });
  ...
  it("POST /api/autoplay-engine-input is disabled unless browser orchestration env is on", async () => {
    ...
    if (prev !== undefined)
      process.env.ADVENTURE_LM_BROWSER_ORCHESTRATED_AUTOPLAY = prev;
  });
  ...
  it("POST /api/prompt-projects creates file in ADVENTURE_LM_PROMPT_PROJECTS_DIR", async () => {
    const dir = mkdtempSync(path.join(tmpdir(), "adv-ppm-"));
    process.env.ADVENTURE_LM_PROMPT_PROJECTS_DIR = dir;
    const server = testHttpDashboard();
    await new Promise<void>((resolve) => {
      server.listen(0, "127.0.0.1", () => resolve());
    });
  ...
  it("GET /api/benchmark-runs/leaderboard returns inserted rows", async () => {
    ...
    process.env.ADVENTURE_LM_BENCHMARK_DB = path.join(dir, "runs.db");
    resetBenchmarkRunsDbSingleton();
