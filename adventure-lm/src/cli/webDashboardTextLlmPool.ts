...
import type { TextLlm, TextLlmProviderId } from "@adventure-lm/lm-glue";
import {
...
export function resolveWebMaxLoadedModels(): number {
  const raw = process.env.ADVENTURE_LM_WEB_MAX_LOADED_MODELS?.trim();
...
export class WebDashboardTextLlmPool {
  ...
  private async makeRoomForNewKeyIfNeeded(newKey: string): Promise<void> {
    while (this.entries.size >= this.maxEntries && !this.entries.has(newKey)) {
      ...
      if (!victim) {
        throw new Error(
          `At most ${this.maxEntries} loaded text model(s) allowed (ADVENTURE_LM_WEB_MAX_LOADED_MODELS).`,
        );
      }
    ...
    if (providerId === "mlx") {
      ...
      try {
        process.stderr.write(
          `adventure-lm: loading MLX model ${modelId} (pooled)…\n`,
        );
