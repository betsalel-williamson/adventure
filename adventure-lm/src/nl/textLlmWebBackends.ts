...
import type { TextLlmProviderId } from "@adventure-lm/lm-glue";
...
/** MLX is swappable when explicitly selected or `ADVENTURE_LM_MLX_MODEL` pins the auto path to MLX-capable config. */
export function mlxBackendAvailableFromEnv(): boolean {
  const raw = process.env.ADVENTURE_LM_TEXT_PROVIDER?.trim().toLowerCase();
  return raw === "mlx" || Boolean(process.env.ADVENTURE_LM_MLX_MODEL?.trim());
}

export function httpBackendAvailableFromEnv(): boolean {
  return Boolean(process.env.ADVENTURE_LM_HTTP_MODEL?.trim());
}
