...
describe("resolveTextLlmFromEnv", () => {
  ...
  beforeEach(() => {
    for (const k of Object.keys(process.env)) {
      if (k.startsWith("ADVENTURE_LM_") || k === "GEMINI_API_KEY") {
  ...
  it("returns HTTP when ADVENTURE_LM_TEXT_PROVIDER=http and model set", () => {
    process.env.ADVENTURE_LM_TEXT_PROVIDER = "http";
    process.env.ADVENTURE_LM_HTTP_MODEL = "gemma2:2b";
    const llm = resolveTextLlmFromEnv();
  ...
  it("returns null when http provider without model", () => {
    process.env.ADVENTURE_LM_TEXT_PROVIDER = "http";
    delete process.env.ADVENTURE_LM_HTTP_MODEL;
    expect(resolveTextLlmFromEnv()).toBeNull();
  });

  it("returns null when google provider without API key", () => {
    process.env.ADVENTURE_LM_TEXT_PROVIDER = "google";
    delete process.env.GEMINI_API_KEY;
    expect(resolveTextLlmFromEnv()).toBeNull();
  });

  it("prefers HTTP when only ADVENTURE_LM_HTTP_MODEL is set (no Google key)", () => {
    process.env.ADVENTURE_LM_HTTP_MODEL = "gemma2:2b";
    const llm = resolveTextLlmFromEnv();
    expect(llm).toBeInstanceOf(HttpOpenAiCompatibleTextLlm);
  });

  it("returns MLX when ADVENTURE_LM_TEXT_PROVIDER=mlx", () => {
  ...
  it("uses MLX when only ADVENTURE_LM_MLX_MODEL is set (no Google, no HTTP)", () => {
    process.env.ADVENTURE_LM_MLX_MODEL = "mlx-community/gemma-2-2b-it";
    delete process.env.GEMINI_API_KEY;
    delete process.env.ADVENTURE_LM_HTTP_MODEL;
    const llm = resolveTextLlmFromEnv();
    expect(llm).toBeInstanceOf(MlxLmStdioTextLlm);
