 ...
 * `ADVENTURE_LM_HTTP_WEB_PRESETS` (comma-separated), then extras from
...
export function mergeHttpWebPresetsFromEnv(): string[] {
  const defaultModel = process.env.ADVENTURE_LM_HTTP_MODEL?.trim();
  const raw = process.env.ADVENTURE_LM_HTTP_WEB_PRESETS?.trim();
  const extra = raw
    ? raw
        .split(",")
        .map((s) => s.trim())
