...
export function mlxLmOptionsFromEnv(
  ...
  const mlxModel =
    ...
    process.env.ADVENTURE_LM_MLX_MODEL?.trim() ||
    DEFAULT_MLX_MODEL_ID;
  const maxTok = process.env.ADVENTURE_LM_MLX_MAX_TOKENS?.trim();
  const readyMs = process.env.ADVENTURE_LM_MLX_READY_TIMEOUT_MS?.trim();
  ...
  const uvEnv = process.env.ADVENTURE_LM_MLX_USE_UV?.trim().toLowerCase();
  ...
  return {
    modelId: mlxModel,
    useUv,
    uvPath: process.env.ADVENTURE_LM_MLX_UV?.trim(),
    packageRoot: process.env.ADVENTURE_LM_MLX_PACKAGE_ROOT?.trim(),
    pythonPath: process.env.ADVENTURE_LM_MLX_PYTHON?.trim(),
    scriptPath: process.env.ADVENTURE_LM_MLX_SCRIPT?.trim(),
    maxTokens,
    readyTimeoutMs,
...
export function createHttpTextLlmFromEnv(
  ...
  if (mid === "") return null;
  const httpBase =
    process.env.ADVENTURE_LM_HTTP_BASE_URL?.trim() ||
    DEFAULT_HTTP_OPENAI_BASE_URL;
  const httpApiKey = process.env.ADVENTURE_LM_HTTP_API_KEY?.trim();
 ...
 *
 * - `ADVENTURE_LM_TEXT_PROVIDER=google` — requires `GEMINI_API_KEY`
 * - `ADVENTURE_LM_TEXT_PROVIDER=http` — requires `ADVENTURE_LM_HTTP_MODEL` (base URL defaults to {@link DEFAULT_HTTP_OPENAI_BASE_URL})
 * - `ADVENTURE_LM_TEXT_PROVIDER=mlx` — Apple Silicon native MLX via **uv** (`uv venv && uv sync` in adventure-lm); optional `ADVENTURE_LM_MLX_MODEL` (defaults to {@link DEFAULT_MLX_MODEL_ID})
 * - Unset: Google if `GEMINI_API_KEY`; else HTTP if `ADVENTURE_LM_HTTP_MODEL`; else MLX if `ADVENTURE_LM_MLX_MODEL`
 */
export function resolveTextLlmFromEnv(): TextLlm | null {
  const rawProvider =
    process.env.ADVENTURE_LM_TEXT_PROVIDER?.trim().toLowerCase();
  const hasGoogleKey = Boolean(process.env.GEMINI_API_KEY?.trim());
  const httpModel = process.env.ADVENTURE_LM_HTTP_MODEL?.trim();
  if (rawProvider === "mlx") {
  ...
  if (rawProvider === undefined || rawProvider === "") {
    ...
    if (process.env.ADVENTURE_LM_MLX_MODEL?.trim()) {
      return createMlxTextLlmFromEnv();
    }
    return null;
  }
