...
export function resolveAutoplayPaceMs(): number {
  const v = envString("ADVENTURE_LM_AUTOPLAY_PACE_MS");
  if (v === undefined || v === "") return 2000;
  const n = Number(v);
  return Number.isFinite(n) && n >= 0 ? n : 2000;
}

/** Default cap when `ADVENTURE_LM_AUTOPLAY_MAX_MOVES` is unset (benchmark-friendly). */
export const DEFAULT_AUTOPLAY_MAX_MOVES = 120;

export function resolveAutoplayMaxMoves(): number {
  const v = envString("ADVENTURE_LM_AUTOPLAY_MAX_MOVES");
...
export function resolveAutoplayContextChars(): number {
  const v = envString("ADVENTURE_LM_AUTOPLAY_CONTEXT_CHARS");
