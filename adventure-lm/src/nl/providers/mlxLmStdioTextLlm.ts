...
export type MlxLmStdioTextLlmOptions = {
   ...
   * Set `ADVENTURE_LM_MLX_USE_UV=0` to use `pythonPath` + script directly.
   */
  useUv?: boolean;
   ...
   * Default: {@link resolveCompactPrompts} for `mlx` (typically true unless `ADVENTURE_LM_COMPACT_PROMPTS=0`).
   */
...
function resolveMlxGenTemp(): number {
  const v = process.env.ADVENTURE_LM_MLX_TEMP?.trim();
...
function resolveMlxStopStrings(): string[] {
  const v = process.env.ADVENTURE_LM_MLX_STOP?.trim();
...
export class MlxLmStdioTextLlm implements TextLlm {
  ...
  private async ensureWorker(): Promise<void> {
    ...
    if (!existsSync(this.resolvedScriptPath)) {
      throw new Error(
        `MLX worker script not found: ${this.resolvedScriptPath}. Install scripts/mlx_lm_worker.py or set ADVENTURE_LM_MLX_SCRIPT.`,
      );
    }
    ...
    if (this.useUv) {
      ...
      if (!existsSync(pyproject)) {
        throw new Error(
          `MLX (uv): missing ${pyproject}. From adventure-lm run: uv venv && uv sync`,
        );
      }
    }
    ...
    this.readyPromise = new Promise((resolve, reject) => {
      ...
      const env = {
        ...process.env,
        ADVENTURE_LM_MLX_MODEL: this.modelId,
      };
  ...
  async interpretPlayerInput(
    ...
    process.stderr.write("adventure-lm: translating with text model (MLX)…\n");
  ...
  async planAutoplay(
    ...
    process.stderr.write(
      "adventure-lm: autoplay — planning next move (MLX)…\n",
    );

    let plannerBody = options.plannerUserPrompt;
    const recent = options.recentGameTextForRepair?.trim() ?? "";
    const twoStep = process.env.ADVENTURE_LM_AUTOPLAY_TWO_STEP?.trim() === "1";
    if (twoStep && recent.length > 0) {
      ...
      if (candidates.length > 0) {
        process.stderr.write(
          "adventure-lm: autoplay — two-step token filter (MLX)…\n",
        );
