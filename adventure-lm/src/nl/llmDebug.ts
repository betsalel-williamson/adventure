...
import type { InterpretedCommand } from "@adventure-lm/lm-glue";
import {
...
/** When set (web dashboard LLM queue), JSONL goes under `.cache/llm-sessions/<id>.jsonl` unless `ADVENTURE_LM_DEBUG_LOG` is set. */
...
/** JSONL log path; set explicitly or enable with `ADVENTURE_LM_DEBUG=1` (defaults under cwd `.cache/`). */
export function resolveDebugLogPath(): string | null {
  const explicit = process.env.ADVENTURE_LM_DEBUG_LOG?.trim();
  if (explicit) return path.resolve(explicit);
  const on = process.env.ADVENTURE_LM_DEBUG?.trim();
...
/** Directory for cached `InterpretedCommand` JSON files (`ADVENTURE_LM_CACHE_DIR`). */
export function resolveCacheDir(): string | null {
  const d = process.env.ADVENTURE_LM_CACHE_DIR?.trim();
...
function resolveDebugMaxPromptChars(): number {
  const v = process.env.ADVENTURE_LM_DEBUG_MAX_PROMPT_CHARS?.trim();
...
/**
 * Truncate long prompt fields before writing JSONL (`ADVENTURE_LM_DEBUG_MAX_PROMPT_CHARS`, default 50000).
 */
