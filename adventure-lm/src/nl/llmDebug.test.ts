...
describe("llmDebug", () => {
  ...
  beforeEach(() => {
    delete process.env.ADVENTURE_LM_DEBUG;
    delete process.env.ADVENTURE_LM_DEBUG_LOG;
  });
  ...
  it("sanitizeInteractionLogRecord truncates long prompt fields", () => {
    const prevMax = process.env.ADVENTURE_LM_DEBUG_MAX_PROMPT_CHARS;
    process.env.ADVENTURE_LM_DEBUG_MAX_PROMPT_CHARS = "40";
    try {
      ...
      if (prevMax === undefined) {
        delete process.env.ADVENTURE_LM_DEBUG_MAX_PROMPT_CHARS;
      } else {
        process.env.ADVENTURE_LM_DEBUG_MAX_PROMPT_CHARS = prevMax;
      }
    }
  });

  it("appendInteractionLog writes JSONL when ADVENTURE_LM_DEBUG_LOG is set", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "adv-llm-log-"));
    const logPath = path.join(dir, "events.jsonl");
    process.env.ADVENTURE_LM_DEBUG_LOG = logPath;
  ...
  it("appendInteractionLog writes per-session JSONL when ADVENTURE_LM_DEBUG and ALS context", async () => {
