...
const BLNK = "    ";
function readI9(line) {
  const field = line.slice(0, 9).trim();
  if (field.length === 0) return 0;
  return parseInt(field, 10);
}
function readA4Chunks(line) {
...
function buildLLineRow(rowIndex, chunks) {
  let kk = 0;
  for (let k = 1; k <= 20; k++) {
    if (chunks[20 - k] !== BLNK) {
      kk = k;
...
function chainAppend(rows, map, jkind, newIndex) {
...
export function loadDatFromString(source) {
...
export function loadDatFile(path) {
  return loadDatFromString(readFileSync(path, "utf8"));
}
//# sourceMappingURL=loadDat.js.map
