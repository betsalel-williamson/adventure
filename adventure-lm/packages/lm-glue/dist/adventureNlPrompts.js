...
import { LLM_PACKAGING_INTERPRET_RECENT_GAME_CHARS_COMPACT, LLM_PACKAGING_INTERPRET_RECENT_GAME_CHARS_FULL, LLM_PACKAGING_PLANNER_PREVIEW_MAX_SYSTEM_CHARS, LLM_PACKAGING_PLANNER_PREVIEW_MAX_USER_CHARS, LLM_PACKAGING_SSE_PROMPT_CAP_CHARS, } from "./llmPackagingConstants.js";
import { buildInterpretEvalExamplesSection, loadInterpretEvalFixtures, } from "./interpretEvalFixtures.js";
// -----------------------------------------------------------------------------
// Single source of instruction text for all TextLlm providers (Google, HTTP, MLX).
// -----------------------------------------------------------------------------
/**
 * Append EXAMPLES from `packages/lm-glue/fixtures/interpret-eval-fixtures.json` (fixtures with
 * `includeInPrompt !== false`) when enabled.
 *
 * - `ADVENTURE_LM_INTERPRET_PROMPT_EXAMPLES=1` / `true` / `yes` → on
 * - `=0` / `false` / `no` → off
...
export function resolveFastInterpretPrompt() {
    const v = process.env.ADVENTURE_LM_FAST_INTERPRET?.trim().toLowerCase();
    return v === "1" || v === "true" || v === "yes";
}
export function resolveInterpretPromptExamples(providerId) {
    if (resolveFastInterpretPrompt())
        return false;
    const v = process.env.ADVENTURE_LM_INTERPRET_PROMPT_EXAMPLES?.trim().toLowerCase();
    if (v === "1" || v === "true" || v === "yes")
        return true;
    if (v === "0" || v === "false" || v === "no")
        return false;
 ...
 */
export const ADVENTURE_LM_INTERPRET_COMPACT_HELP_HINT = "If the user asks for instructions, hints, or how to play, use primaryToken HELP and omit secondaryToken.";
 ...
 */
export function resolveInterpretPromptBuildOptions(providerId, overrides) {
    return {
        compact: overrides?.compact !== undefined
            ? overrides.compact
            : resolveCompactPrompts(providerId),
        structuredDashboard: overrides?.structuredDashboard !== undefined
...
export const ADVENTURE_LM_INTERPRET_ROLE = "You are mapping user input to Colossal Cave Adventure parser tokens (max 5 letters each, like the original game).";
/** Shorter role for small local models (MLX): less preamble, same task. */
export const ADVENTURE_LM_INTERPRET_ROLE_COMPACT = "Map the user's line to Colossal Cave Adventure parser tokens (max 5 letters each). Reply with JSON only.";
/**
...
export const ADVENTURE_LM_AUTOPLAY_LOOT_FUNNEL_SYSTEM_SLM = `**Loot gate (this turn):** **Items** lists ground objects you are not yet carrying. You MUST output {"primaryToken":"TAKE","secondaryToken":"<item>"} (or GET, same shape) for one of those objects. Do not emit travel-only JSON until **Items** is (none visible). **Cand_Move** is hidden — use **Cand_Act**/**Cand_Obj** only.`;
/** Autoplay: role for the planner + session-memory context (same rules as interpret). */
export const ADVENTURE_LM_AUTOPLAY_PLANNER_ROLE = "You are playing Colossal Cave Adventure as the adventurer. Choose the next parser command to continue. Reply ONLY with JSON; the exact key list is specified again at the end of the full prompt after this context.";
export const ADVENTURE_LM_AUTOPLAY_PLANNER_ROLE_COMPACT = "You are the adventurer. Choose the next parser command. Reply ONLY with JSON; keys are repeated at the end of the prompt.";
/**
 * `ADVENTURE_LM_AUTOPLAY_PROMPT_MODE=explore` (default) or `full`.
 * Explore mode prioritizes visiting new rooms and varying commands; full preserves longer loot/indoor hints.
 */
export function resolveAutoplayPromptMode() {
    const v = process.env.ADVENTURE_LM_AUTOPLAY_PROMPT_MODE?.trim().toLowerCase();
    if (v === "full")
        return "full";
    return "explore";
}
 ...
 */
export function mlxAutoplayPlannerInstructionsBlock(compact, mode = resolveAutoplayPromptMode()) {
    if (compact) {
...
Use only primaryToken + secondaryToken when needed. Shorter JSON is better.`;
    ...
    const rules = ADVENTURE_LM_PARSER_TOKEN_RULES;
 ...
 * Set `ADVENTURE_LM_MLX_SYSTEM_VARIANT` to `full` | `compact` | `core` | `bare` (default `full`).
 */
export function resolveMlxSystemPromptVariant() {
    const v = process.env.ADVENTURE_LM_MLX_SYSTEM_VARIANT?.trim().toLowerCase();
...
function mlxAutoplaySystemPromptBare(compact) {
    const rules = compact
        ? ADVENTURE_LM_AUTOPLAY_RULES_SLM
 ...
 */
export function mlxAutoplaySystemPromptForVariant(compact, variant, mode = resolveAutoplayPromptMode(), lootFunnelActive) {
...
export function mlxAutoplaySystemPrompt(compact, mode = resolveAutoplayPromptMode(), lootFunnelActive) {
    return mlxAutoplaySystemPromptForVariant(compact, resolveMlxSystemPromptVariant(), mode, lootFunnelActive);
}
/**
 ...
 */
export function appendFlatVocabularyToPlannerSystem(systemBase, vocabularyFlat) {
...
export function recentGameCharsCapForInterpret(compact) {
    return compact
 ...
 */
export function recentGameTextSliceForInterpretPrompt(recentGameText, compact) {
 ...
 * Set `ADVENTURE_LM_COMPACT_PROMPTS=0` to use full prompts on MLX, or `=1` to force compact on any provider.
 */
export function resolveCompactPrompts(providerId) {
    const v = process.env.ADVENTURE_LM_COMPACT_PROMPTS?.trim().toLowerCase();
    if (v === "0" || v === "false" || v === "no")
        return false;
    if (v === "1" || v === "true" || v === "yes")
        return true;
 ...
 * Default **on** for MLX only. Set `ADVENTURE_LM_STRUCTURED_PROMPTS=0` to disable on MLX.
 */
export function resolveStructuredDashboardPrompts(providerId) {
    const v = process.env.ADVENTURE_LM_STRUCTURED_PROMPTS?.trim().toLowerCase();
    if (v === "0" || v === "false" || v === "no")
        return false;
    if (v === "1" || v === "true" || v === "yes")
        return true;
...
export function autoplayPlannerTaskBlockStructured(mode = resolveAutoplayPromptMode()) {
    if (mode === "explore") {
        return `### TASK
...
export function interpretTaskBlockStructured() {
    return `### TASK
 ...
 * Override with `ADVENTURE_LM_VOCAB_HINT_MAX` (8–500).
 * When `compact` is true and env is unset, uses fewer words (48 vs 120).
 */
export function resolveVocabHintMaxWords(compact) {
    const v = process.env.ADVENTURE_LM_VOCAB_HINT_MAX?.trim();
    if (v !== undefined && v !== "") {
        const n = Number(v);
        if (Number.isFinite(n) && n >= 8 && n <= 500)
            return Math.floor(n);
    }
    return compact ? 48 : 120;
}
/** Footer for interpret prompts (appended after user line + rules). */
export function adventureLlmInterpretJsonFooter() {
...
export function adventureLlmAutoplayJsonFooter() {
 ...
 */
export function linesForAutoplayPlannerContextBody(vocabSectionText, options) {
    ...
    const vocabHeader = structured
        ...
        : compact
            ? "## Vocabulary (parser tokens)"
            : "## Vocabulary (words the game parser accepts, grouped by kind)";
    const exploreLine = mode === "explore"
...
export function buildInterpretSystemAndUserPrompt(db, userText, recentGameText, options) {
    const compact = options?.compact ?? false;
    const structured = options?.structuredDashboard ?? false;
    ...
    const helpBlock = !compact && helpFromDat.length > 0
        ...
        : "";
    const compactHelpBlock = compact && helpFromDat.length > 0
        ...
        : "";
    const recentSlice = recentGameTextSliceForInterpretPrompt(recentGameText, compact);
    const role = compact
    ...
    const examplesSection = resolveInterpretPromptExamples(providerId)
        ...
        : "";
    const examplesSep = examplesSection.length > 0 ? `${examplesSection}\n\n` : "";
...
${adventureLlmInterpretJsonFooter()}`;
    ...
    const recentBlock = recentSlice.length > 0
 ...
 */
export function buildAutoplayPlannerPromptParts(db, parts, options) {
    const compact = options?.compact ?? false;
    const includeHelp = options?.includeDatHelpInSystem !== false;
    const helpFromDat = getHelpInstructionText(db);
    const helpBlock = includeHelp && !compact && helpFromDat.length > 0
 ...
 */
export function buildAutoplayPlannerPrompt(db, plannerUserPrompt, options) {
    const compact = options?.compact ?? false;
    const includeHelp = options?.includeDatHelpInSystem !== false;
...
${adventureLlmAutoplayJsonFooter()}`;
    ...
    const merged = buildAutoplayPlannerPromptParts(db, plannerUserPrompt, options);
    return `${merged.system}\n\n${merged.user}`;
}
function capForSse(s) {
    if (s.length <= LLM_PACKAGING_SSE_PROMPT_CAP_CHARS)
        return s;
 ...
 */
export function effectivePlannerSendPayload(db, providerId, plannerUserPrompt, options) {
    const buildOpts = {
        compact: options.compact,
        includeDatHelpInSystem: options.includeDatHelpInSystem,
    };
    const cap = options.capForEventStream === true;
    const wrap = (x) => (cap ? capForSse(x) : x);
    if (providerId === "mlx" &&
        options.structuredSplit &&
        typeof plannerUserPrompt === "object") {
        const parts = buildAutoplayPlannerPromptParts(db, plannerUserPrompt, buildOpts);
        ...
        return {
            ...
            fullUser,
            merged: wrap(merged),
            userPreview: parts.user.length <= maxU
                ? parts.user
                : `${parts.user.slice(0, maxU)}…`,
            systemPreview: parts.system.length <= maxS
                ? parts.system
                : `${parts.system.slice(0, maxS)}…`,
        };
    }
    const mergedRaw = buildAutoplayPlannerPrompt(db, plannerUserPrompt, buildOpts);
    const merged = wrap(mergedRaw);
    const maxU = LLM_PACKAGING_PLANNER_PREVIEW_MAX_USER_CHARS;
    return {
        merged,
        userPreview: mergedRaw.length <= maxU ? mergedRaw : `${mergedRaw.slice(0, maxU)}…`,
    };
}
//# sourceMappingURL=adventureNlPrompts.js.map