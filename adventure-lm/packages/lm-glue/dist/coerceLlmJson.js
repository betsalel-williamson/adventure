...
function coerceSecondaryToken(v) {
    if (v === null || v === undefined)
        return undefined;
    const s = String(v).trim().toUpperCase();
    if (s === "" || s === "NULL" || s === "NONE" || s === "N/A")
        return undefined;
    const first = s.split(/\s+/)[0] ?? "";
    const letters = first.replace(/[^A-Z]/g, "");
    const t = letters.slice(0, 5);
...
export function coerceInterpretedCommandJson(raw) {
...
export function coerceAutoplayPlannerJson(raw) {
...
//# sourceMappingURL=coerceLlmJson.js.map