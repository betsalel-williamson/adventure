...
function resolveHttpRetryMaxAttempts(): number {
  const v = process.env.ADVENTURE_NL_HTTP_RETRY_ATTEMPTS?.trim();
...
export class HttpOpenAiCompatibleTextLlm implements TextLlm {
  ...
  async interpretPlayerInput(
    ...
    process.stderr.write("adventure-nl: translating with text model…\n");
  ...
  async planAutoplay(
    ...
    process.stderr.write("adventure-nl: autoplay — planning next move…\n");
