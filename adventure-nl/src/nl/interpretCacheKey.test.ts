...
describe("interpretCacheKeyFromBuildOptions", () => {
  const prev = process.env.ADVENTURE_NL_CACHE_SCHEMA_VERSION;

  beforeEach(() => {
    delete process.env.ADVENTURE_NL_CACHE_SCHEMA_VERSION;
  });

  afterEach(() => {
    if (prev === undefined)
      delete process.env.ADVENTURE_NL_CACHE_SCHEMA_VERSION;
    else process.env.ADVENTURE_NL_CACHE_SCHEMA_VERSION = prev;
  });
