  ...
  return {
    ...
    jsonSchemaByMode: {
      interpret: {
        wire: httpWire,
        notes: httpJson
          ? "OpenAI-compatible `response_format.type=json_schema` with strict schema from `openAiInterpretCommandJsonSchema` when `ADVENTURE_NL_HTTP_JSON_SCHEMA` is enabled."
