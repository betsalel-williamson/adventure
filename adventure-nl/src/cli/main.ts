 ...
 * logging (see ADVENTURE_NL_DEBUG* and ADVENTURE_NL_CACHE_DIR in .env.example).
 * Pass --autoplay for self-acting mode (model-driven moves; see ADVENTURE_NL_AUTOPLAY_* in .env.example).
...
import { AutoplaySessionMemory } from "@adventure-nl/nl-glue";
import {
  interpretWithTextLlm,
  resolveTextLlmFromEnv,
} from "../nl/adventureTextLlm.js";
import { resolveInterpretPromptBuildOptions } from "@adventure-nl/nl-glue";
...
import { instructionIntentToHelpCommand } from "@adventure-nl/nl-glue";
import {
  interpretedToGetinLine,
  swapInterpretedTokens,
} from "@adventure-nl/nl-glue";
import type { TextLlm } from "@adventure-nl/nl-glue";
...
/** dist/cli -> adventure-nl -> repo root */
...
function printClassicBanner(
  ...
  process.stderr.write(
    "adventure-nl — classic mode uses the original Fortran Colossal Cave engine.\n",
  );
  if (!hasTextLlmConfigured) {
    process.stderr.write(
      "Natural language needs a text model: set GEMINI_API_KEY and/or ADVENTURE_NL_HTTP_* (see adventure-nl/.env.example). Omit --classic once configured.\n",
    );
...
async function ensureMlxWorkerReady(client: TextLlm): Promise<void> {
  ...
  process.stderr.write("adventure-nl: loading MLX model (one-time)…\n");
  await client.preloadWorker();
}

/** Default on: heuristic session block + situational tokens for interactive NL (set ADVENTURE_NL_INTERACTIVE_SESSION=0 to disable). */
function resolveInteractiveSessionMemoryEnabled(): boolean {
  const v = process.env.ADVENTURE_NL_INTERACTIVE_SESSION?.trim().toLowerCase();
...
async function main(): Promise<void> {
  ...
  if (autoplay && !hasTextLlm) {
    process.stderr.write(
      "adventure-nl: --autoplay requires a text model (GEMINI_API_KEY and/or ADVENTURE_NL_HTTP_MODEL; see .env.example).\n",
    );
    process.exitCode = 1;
    return;
  }
  if (autoplay && forceClassic) {
    process.stderr.write(
      "adventure-nl: --autoplay cannot be used with --classic.\n",
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (autoplay && textLlm) {
    ...
    try {
      ...
      if (shouldFallbackToClassicForLlmError(err, textLlm.providerId)) {
        process.stderr.write(
          "\nadventure-nl: text model unavailable (quota, rate limit, network, or service error). Autoplay stopped.\n",
        );
        process.exitCode = 1;
        return;
      }
  ...
  process.stderr.write(
    "adventure-nl: language-model-assisted input enabled.\n",
  );
  process.stderr.write(
    "adventure-nl: type naturally at > ; input is translated to game vocabulary. .quit or :q ends the session.\n",
  );
  const logPath = resolveDebugLogPath();
  if (logPath) {
    process.stderr.write(`adventure-nl: interaction log → ${logPath}\n`);
  }
  const cacheDir = resolveCacheDir();
  if (cacheDir) {
    process.stderr.write(`adventure-nl: response cache → ${cacheDir}\n`);
  }
  ...
  try {
    ...
    if (shouldFallbackToClassicForLlmError(err, textLlm!.providerId)) {
      process.stderr.write(
        "\nadventure-nl: text model is unavailable (quota, rate limit, or service error). Switching to classic mode: type parser words directly (e.g. EAST, TAKE LAMP).\n\n",
      );
      printClassicBanner(hasTextLlm, forceClassic);
      runClassicInteractive();
      return;
