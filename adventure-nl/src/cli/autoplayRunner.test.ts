...
describe("resolveAutoplayMaxMoves", () => {
  it("defaults to DEFAULT_AUTOPLAY_MAX_MOVES when env unset", () => {
    const prev = process.env.ADVENTURE_NL_AUTOPLAY_MAX_MOVES;
    delete process.env.ADVENTURE_NL_AUTOPLAY_MAX_MOVES;
    expect(resolveAutoplayMaxMoves()).toBe(DEFAULT_AUTOPLAY_MAX_MOVES);
    if (prev === undefined) delete process.env.ADVENTURE_NL_AUTOPLAY_MAX_MOVES;
    else process.env.ADVENTURE_NL_AUTOPLAY_MAX_MOVES = prev;
  });
});
