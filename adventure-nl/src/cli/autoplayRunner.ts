...
function logAutoplaySendingToGame(
  ...
  const lineOut = `adventure-nl: autoplay — planned ${tok.join(", ")} → to adventure (GETIN): ${getinDesc}\n`;
  log(lineOut);
}
...
/** Instructions screen: `ADVENTURE_NL_INSTRUCTIONS=y` or `n` (default `n`). */
export function resolveAutoplayInstructionsAnswer(): string {
  const v = process.env.ADVENTURE_NL_INSTRUCTIONS?.trim().toLowerCase();
...
export async function runAutoplaySessionWithTextLlm(
  ...
  const callPlanner = async (recentForRepair: string) => {
    ...
    const capSse = process.env.ADVENTURE_NL_SSE_FULL_PROMPTS?.trim() === "1";
  ...
  if (sink === undefined) {
    process.stderr.write("adventure-nl: self-acting (autoplay) mode.\n");
    process.stderr.write(
      `adventure-nl: pace ${paceMs0}ms; max moves ${maxMoves0}; context ~${contextChars} chars.\n`,
    );
  }
  ...
  await runFortranOpenThenFirstCommand({
    ...
    getContinueLine: async (ctx) => {
      ...
      if (gameOutputLooksLikePlayAgainPrompt(ctx.gameOutputSinceLastCommand)) {
        ...
        logLine(
          "adventure-nl: autoplay — GAME IS OVER / PLAY AGAIN detected; sending Y (restart)\n",
        );
        logAutoplaySendingToGame(planForLog, scripted, logLine);
