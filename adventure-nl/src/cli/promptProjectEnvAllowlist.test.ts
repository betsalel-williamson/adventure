...
describe("promptProjectEnvAllowlist", () => {
  ...
  it("sanitize drops unknown keys", () => {
    const o = sanitizePromptProjectEnvAllowlist({
      ADVENTURE_NL_AUTOPLAY_PROMPT_MODE: "full",
      EVIL_KEY: "x",
    });
    expect(o).toEqual({ ADVENTURE_NL_AUTOPLAY_PROMPT_MODE: "full" });
  });
  ...
  it("apply and restore round-trip", () => {
    ...
    const b = applyAllowlistedEnvToProcess({
      ADVENTURE_NL_AUTOPLAY_PROMPT_MODE: "explore",
    });
    expect(process.env.ADVENTURE_NL_AUTOPLAY_PROMPT_MODE).toBe("explore");
