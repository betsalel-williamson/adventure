...
const repoFixturesPath = path.join(
  ...
  "../../../packages/nl-glue/fixtures/interpret-eval-fixtures.json",
);
...
describe("buildInterpretEvalExamplesSection", () => {
  ...
  it("returns empty when all fixtures opt out of prompt", () => {
    ...
    expect(
      buildInterpretEvalExamplesSection(onlyOut, {
  ...
  it("uses ### EXAMPLES when structured", () => {
    const text = buildInterpretEvalExamplesSection(
      [{ ...sample[0]!, includeInPrompt: true }],
      { structuredDashboard: true },
    );
