...
describe("resolveCompactPrompts", () => {
  ...
  beforeEach(() => {
    delete process.env.ADVENTURE_NL_COMPACT_PROMPTS;
  });
  ...
  it("respects ADVENTURE_NL_COMPACT_PROMPTS=0 and =1", () => {
    process.env.ADVENTURE_NL_COMPACT_PROMPTS = "0";
    expect(resolveCompactPrompts("mlx")).toBe(false);
    process.env.ADVENTURE_NL_COMPACT_PROMPTS = "1";
...
describe("resolveStructuredDashboardPrompts", () => {
  ...
  beforeEach(() => {
    delete process.env.ADVENTURE_NL_STRUCTURED_PROMPTS;
  });
  ...
  it("respects ADVENTURE_NL_STRUCTURED_PROMPTS", () => {
    process.env.ADVENTURE_NL_STRUCTURED_PROMPTS = "0";
    expect(resolveStructuredDashboardPrompts("mlx")).toBe(false);
    process.env.ADVENTURE_NL_STRUCTURED_PROMPTS = "1";
    expect(resolveStructuredDashboardPrompts("google")).toBe(true);
  });
});

describe("MLX system prompt variants (ADVENTURE_NL_MLX_SYSTEM_VARIANT)", () => {
  const prev = { ...process.env };
...
describe("resolveVocabHintMaxWords", () => {
  ...
  it("uses fewer words when compact and env unset", () => {
    delete process.env.ADVENTURE_NL_VOCAB_HINT_MAX;
    expect(resolveVocabHintMaxWords(true)).toBe(48);
    expect(resolveVocabHintMaxWords(false)).toBe(120);
  });

  it("respects ADVENTURE_NL_VOCAB_HINT_MAX", () => {
    process.env.ADVENTURE_NL_VOCAB_HINT_MAX = "64";
...
describe("resolveInterpretPromptExamples (ADVENTURE_NL_INTERPRET_PROMPT_EXAMPLES)", () => {
  const prev = { ...process.env };
  ...
  beforeEach(() => {
    ...
    delete process.env.ADVENTURE_NL_VOCAB_AI_CATEGORIES;
    delete process.env.ADVENTURE_NL_FAST_INTERPRET;
  });
  ...
  it("ADVENTURE_NL_FAST_INTERPRET=1 disables examples for mlx", () => {
    delete process.env.ADVENTURE_NL_INTERPRET_PROMPT_EXAMPLES;
    process.env.ADVENTURE_NL_FAST_INTERPRET = "1";
  ...
  it("ADVENTURE_NL_INTERPRET_PROMPT_EXAMPLES=0 disables for mlx", () => {
    process.env.ADVENTURE_NL_INTERPRET_PROMPT_EXAMPLES = "0";
    expect(resolveInterpretPromptExamples("mlx")).toBe(false);
  });

  it("when enabled, structured prompt includes ### EXAMPLES before ### USER INPUT", () => {
    process.env.ADVENTURE_NL_INTERPRET_PROMPT_EXAMPLES = "1";
    const db = loadDatFile(datPath);
