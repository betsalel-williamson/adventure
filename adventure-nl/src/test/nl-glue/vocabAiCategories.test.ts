...
describe("loadAiVocabCategoriesForHint", () => {
  ...
  beforeEach(() => {
    ...
    process.env = { ...prev };
    delete process.env.ADVENTURE_NL_VOCAB_CATEGORIES_FILE;
...
describe("buildVocabHint with AI file", () => {
  ...
  beforeEach(() => {
    ...
    const mid = Math.ceil(all.length / 2);
    writeFileSync(
      process.env.ADVENTURE_NL_VOCAB_CATEGORIES_FILE,
      JSON.stringify({
        version: 1,
        groups: [
          {
...
describe("resolveAiVocabCategoriesPath", () => {
  ...
  it("honors ADVENTURE_NL_VOCAB_CATEGORIES_FILE", () => {
    process.env.ADVENTURE_NL_VOCAB_CATEGORIES_FILE = "/tmp/custom.json";
