import { DIAGONAL_COMPASS_MOTION_TOKEN_SET, isDiagonalCompassMotionEnabled, } from "./diagonalCompassMotion.js";
...
const CLASS_VERB = 3;
function skipDiagonalMotionToken(token) {
    return (!isDiagonalCompassMotionEnabled() &&
 ...
 */
export function recentTextSuggestsIndoorBuildingNavigation(text) {
    const u = text.replace(/\r\n/g, "\n").toUpperCase();
    if (u.includes("YOU'RE INSIDE BUILDING"))
        return true;
    if (u.includes("YOU ARE INSIDE A BUILDING"))
        return true;
    if (u.includes("YOU ARE INSIDE") && u.includes("WELL HOUSE"))
        return true;
    if (u.includes("NO WAY TO GO THAT DIRECTION") &&
 ...
 */
export function recentTextSuggestsVerticalPassageNavigation(text) {
    const u = text.replace(/\r\n/g, "\n").toUpperCase();
    const hasWord = (re) => re.test(u);
    // --- Grate / stream depression ---
    if (u.includes("GRATE")) {
        if (u.includes("OUTSIDE GRATE"))
            return true;
        if (u.includes("METAL GRATE"))
            return true;
        if (u.includes("STEEL GRATE"))
            return true;
        if (u.includes("STREAM") && u.includes("GRATE"))
            return true;
        if (u.includes("DEPRESSION") && u.includes("GRATE"))
            return true;
        if (u.includes("TWENTY FOOT") && u.includes("GRATE"))
            return true;
        if (u.includes("TWENTY-FOOT") && u.includes("GRATE"))
            return true;
    ...
    // --- Stairs / ladder ---
    if (hasWord(/\b(LADDER|STAIRS?|STEPS)\b/))
        return true;
    // --- Shaft / well (vertical) ---
...
export function recentTextSuggestsGrateDescentNavigation(text) {
...
const INDOOR_EXIT_MOTION_FIRST = [
    "OUT",
    "BUILD",
    "LEAVE",
    "EXIT",
...
const VERTICAL_PASSAGE_MOTION_FIRST = ["DOWN", "UP"];
const COMPASS_MOTION_DEPRIORITIZE = [
    "NORTH",
    "SOUTH",
    "EAST",
    "WEST",
 ...
 */
const DEFER_OBSERVATION_MOTION_TO_TAIL = ["LOOK", "EXAMI"];
...
function tokenizeUpperWords(text) {
    const set = new Set();
 ...
 */
export function stripInjectedCommandLinesForObjectHints(text) {
    return text
        .replace(/\r\n/g, "\n")
        .split("\n")
...
function objectClassWordsFromDat(db) {
    ...
    for (let i = 1; i < 1000; i++) {
        ...
        if (w.length === 0)
            continue;
        if (ktabClass(db.ktab[i]) === CLASS_OBJECT)
            objectSet.set(w, w);
    }
    return objectSet;
}
 ...
 * (same token matching as {@link buildSituationalCandidateTokens}).
 */
export function countVisibleAdventureObjectsInText(db, text) {
 ...
 */
export function matchSecondaryToObjectAtabWord(secondary, db) {
    const t = secondary.trim().toUpperCase();
    if (t.length === 0)
        return undefined;
    const objectSet = objectClassWordsFromDat(db);
    const head = t.slice(0, 5);
    if (objectSet.has(head))
        return head;
    for (const w of objectSet.keys()) {
        if (w.slice(0, 5) === head)
            return w;
...
const LOOT_FUNNEL_KEEP_MOTION_CLASS1 = new Set([
    "LOOK",
    "EXAMI",
]);
/** Class-1 travel words allowed through loot funnel when vertical-passage context applies. */
const LOOT_FUNNEL_VERTICAL_PASSAGE_MOTION = new Set([
    "DOWN",
    "UP",
]);
/**
 ...
 */
export function listVisibleRoomObjectsNotCarried(db, recentGameText, structuredInventoryLines, 
...
takeFailureAtabWords) {
    ...
    const carried = invJoined.length > 0
        ? new Set(listVisibleAdventureObjectsInText(db, invJoined))
        : new Set();
    const failed = new Set((takeFailureAtabWords ?? [])
 ...
 */
export function shouldPrioritizeLootFunnel(db, recentGameText, structuredInventoryLines, takeFailureAtabWords) {
    return (listVisibleRoomObjectsNotCarried(db, recentGameText, structuredInventoryLines, takeFailureAtabWords).length > 0);
}
export function buildSituationalCandidateTokens(db, recentGameText, options) {
    const maxTotal = options?.maxTotal ?? DEFAULT_MAX_TOTAL;
    const deprioritize = new Set((options?.deprioritize ?? []).map((t) => t.trim().toUpperCase()));
    if (options?.indoorLeaveBuilding) {
        for (const c of COMPASS_MOTION_DEPRIORITIZE)
            deprioritize.add(c);
    }
    const motion = [];
    const objects = [];
    ...
    const misc = [];
    const seen = new Set();
    const motionSet = new Map();
    const objectSet = objectClassWordsFromDat(db);
    const verbSet = new Map();
    const miscSet = new Map();
    for (let i = 1; i < 1000; i++) {
        if (db.ktab[i] === 0 && db.atab[i].trim() === "")
            break;
        if (db.ktab[i] === -1)
        ...
        else if (c === CLASS_VERB)
            verbSet.set(w, w);
        else if (c === CLASS_MISC)
            miscSet.set(w, w);
    }
    ...
    const scopedSource = scopeOpt !== undefined && scopeOpt.trim().length > 0
        ? scopeOpt
        : recentGameText;
    const textForObjects = stripInjectedCommandLinesForObjectHints(scopedSource);
    const verticalPassageNav = recentTextSuggestsVerticalPassageNavigation(textForObjects);
    const tokensInText = tokenizeUpperWords(textForObjects);
    const carriedFromInventory = new Set();
    ...
    const takeFailSubtract = new Set();
    for (const raw of options?.takeFailureSubtractWords ?? []) {
        const w = raw.trim().toUpperCase();
        if (w.length > 0)
            takeFailSubtract.add(w);
    }
    let visibleRoomObjectCount = 0;
    for (const w of objectSet.keys()) {
        if (carriedFromInventory.has(w))
            continue;
        if (takeFailSubtract.has(w))
            continue;
        if (objectAtabWordInGameText(w, tokensInText))
            visibleRoomObjectCount++;
    }
    const motionCap = visibleRoomObjectCount > 0 && !exploreFirst
    ...
    for (const p of MOTION_PRIORITY) {
        if (skipDiagonalMotionToken(p))
            continue;
        if (deferMotion.has(p))
            continue;
        if (motionSet.has(p))
            push(motion, p);
        if (motion.length >= motionCap)
            break;
    }
    for (const w of [...motionSet.keys()].sort((a, b) => a.localeCompare(b))) {
        if (motion.length >= motionCap)
            break;
        if (skipDiagonalMotionToken(w))
            continue;
        if (deferMotion.has(w))
            continue;
        if (!seen.has(w))
            push(motion, w);
    }
    ...
    for (const p of MISC_PRIORITY) {
        if (miscSet.has(p))
            push(misc, p);
        if (misc.length >= MAX_MISC)
            break;
    }
    const out = [];
    let motionWorking = motion;
    if (options?.indoorLeaveBuilding) {
        const exitFirst = INDOOR_EXIT_MOTION_FIRST.filter((x) => motionWorking.includes(x));
    ...
    if (verticalPassageNav) {
        const verticalFirst = VERTICAL_PASSAGE_MOTION_FIRST.filter((x) => motionWorking.includes(x));
    ...
    if (lootFunnel && roomListsTakeableObjects) {
        const filtered = out.filter((raw) => {
            ...
            if (LOOT_FUNNEL_KEEP_MOTION_CLASS1.has(t))
                return true;
            if (verticalPassageNav &&
 ...
 */
function bucketCandidatesForSlmPlanner(db, candidates) {
    const move = [];
    const act = [];
    const obj = [];
    const sm = new Set();
    ...
    for (const raw of candidates) {
        ...
        if (cls === CLASS_OBJECT)
            push(obj, so, t);
        else if (cls === CLASS_MOTION)
            push(move, sm, t);
        else if (cls === CLASS_VERB)
            push(act, sa, t);
        else if (cls === CLASS_MISC)
            push(act, sa, t);
        else
            push(move, sm, t);
    }
...
export function formatSituationalCandidatesSection(db, candidates, appendix, options) {
    const flatList = options?.flatList === true;
    const slmGrouped = options?.slmGrouped === true;
    const appendixBlock = appendix !== undefined && appendix.trim().length > 0
        ? `\n\n${appendix.trim()}`
        : "";
...
(no narrowed list — use vocabulary and transcript below)${appendixBlock}`;
    ...
    if (slmGrouped) {
        ...
        const moveDisplay = move.length > 0
            ? move.join(", ")
    ...
    const pushBullets = (title, tokens) => {
        if (tokens.length === 0)
            return;
        lines.push("", title);
    ...
    if (hasPickupStory) {
        lines.push("", "### Pick up items in this room (before leaving)", "If the room lists items you are not carrying, **TAKE** or **GET** + object usually comes **before** exit/travel.");
 ...
 */
export function buildAutoplayRelevantTokensFilterPrompt(recentGameText, allowedTokens) {
...
export function parseRelevantTokensResponse(raw, allowed) {
...
//# sourceMappingURL=situationalCandidates.js.map