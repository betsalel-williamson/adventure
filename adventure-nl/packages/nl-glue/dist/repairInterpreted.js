import { DIAGONAL_COMPASS_MOTION_TOKENS, isDiagonalCompassMotionEnabled, } from "./diagonalCompassMotion.js";
function motionOnlyPrimarySet() {
    const base = [
        "UP",
        "DOWN",
        "N",
...
function stripBadSecondary(s) {
    if (s === undefined)
        return undefined;
    const t = s.trim().toUpperCase();
    if (t === "" || t === "NULL" || t === "NONE" || t === "N/A")
        return undefined;
    return t.slice(0, 5);
}
/**
 ...
 */
export function repairInterpretedCommand(userText, cmd, recentGameText) {
    ...
    if (!secondary && /\b(take|get|grab|snatch)\b/.test(u)) {
        const om = u.match(/\b(keys|lamp|bottle|bird|cage|rod|rug|food|water|egg|coin|diamond|emerald)\b/);
        if (om) {
    ...
    const pickUpPhrasal = /\bpick\b/.test(u) &&
        /\bup\b/.test(u) &&
        !/\bgo\b\s+up\b/.test(u) &&
...
function inferObjectFromGameText(gameUpper) {
    const withThe = gameUpper.match(/WITH THE (KEYS|LAMP|BOTTLE|BIRD|CAGE|ROD)/);
    if (withThe)
        return withThe[1];
    const m = gameUpper.match(/\b(KEYS|LAMP|BOTTLE|BIRD|CAGE|ROD|RUG|CHAIN|FOOD|WATER|COIN)\b/);
    return m?.[1];
}
function inferObjectFromUserText(u) {
    const m = u.match(/\b(keys|lamp|bottle|bird|cage|rod)\b/);
    return m ? m[1].toUpperCase().slice(0, 5) : undefined;
}
//# sourceMappingURL=repairInterpreted.js.map