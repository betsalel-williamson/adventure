...
import { buildSituationalCandidateTokens, countVisibleAdventureObjectsInText, formatSituationalCandidatesSection, parseRelevantTokensResponse, recentTextSuggestsGrateDescentNavigation, recentTextSuggestsIndoorBuildingNavigation, recentTextSuggestsVerticalPassageNavigation, listVisibleRoomObjectsNotCarried, shouldPrioritizeLootFunnel, stripInjectedCommandLinesForObjectHints, } from "@adventure-llm/nl-glue";
...
describe("countVisibleAdventureObjectsInText", () => {
    it("counts distinct object-class words present in prose", () => {
        ...
        const text = "YOU ARE IN A BUILDING. THERE ARE SOME KEYS ON THE GROUND. A BRASS LAMP IS NEARBY. AN EMPTY BOTTLE HERE.";
...
describe("buildSituationalCandidateTokens", () => {
    it("includes directions and verbs, and objects mentioned in text", () => {
        const db = loadDatFile(datPath);
        const text = "YOU ARE IN A BUILDING. THERE ARE SOME KEYS ON THE GROUND. A BRASS LAMP IS NEARBY.";
        const c = buildSituationalCandidateTokens(db, text, { maxTotal: 80 });
        expect(c).toContain("EAST");
        expect(c).toContain("TAKE");
        expect(c.some((w) => w === "KEYS" || w === "KEY" || w === "LAMP")).toBe(true);
    });
    ...
    it("matches ATAB object words to longer prose (BOTTL ↔ BOTTLE)", () => {
        const db = loadDatFile(datPath);
        const text = "YOU'RE INSIDE BUILDING. THERE IS A SHINY BRASS LAMP NEARBY. THERE IS AN EMPTY BOTTLE HERE.";
        const c = buildSituationalCandidateTokens(db, text, { maxTotal: 80 });
        ...
        expect(idxTake).toBeGreaterThanOrEqual(0);
        if (idxOut >= 0)
            expect(idxTake).toBeLessThan(idxOut);
    });
    ...
    it("indoorLeaveBuilding lists OUT before NORTH when both present", () => {
        const db = loadDatFile(datPath);
        const text = "THERE IS NO WAY TO GO THAT DIRECTION. YOU'RE INSIDE BUILDING. LAMP.";
    ...
    it("grate context lists DOWN before compass when room objects are present", () => {
        const db = loadDatFile(datPath);
        const text = "YOU'RE OUTSIDE GRATE. THERE IS WATER HERE. A METAL GRATE IN THE STREAM.\n";
        ...
        if (iEast >= 0)
            expect(iDown).toBeLessThan(iEast);
    });
    it("objectHintScopeText ignores stale GRATE from an earlier room in the tail", () => {
        ...
        const forestOnly = "YOU'RE IN OPEN FOREST, WITH A DEEP VALLEY IN ONE DIRECTION.";
        ...
        expect(shouldPrioritizeLootFunnel(db, forestOnly, [], undefined)).toBe(false);
    ...
    it("lootFunnel retains DOWN/UP when vertical passage context (travel otherwise hidden)", () => {
        const db = loadDatFile(datPath);
        const text = "YOU'RE OUTSIDE GRATE. SMALL STREAM. WATER HERE. METAL GRATE.\n";
        const c = buildSituationalCandidateTokens(db, text, {
            maxTotal: 80,
            lootFunnel: true,
        });
    ...
    it("shouldPrioritizeLootFunnel matches room objects minus carried (loot gate)", () => {
        ...
        expect(shouldPrioritizeLootFunnel(db, text, [
            "YOU ARE CARRYING SOME KEYS",
            "YOU ARE CARRYING A LAMP",
        ])).toBe(false);
        expect(shouldPrioritizeLootFunnel(db, text, ["YOU ARE CARRYING SOME KEYS"])).toBe(true);
        expect(listVisibleRoomObjectsNotCarried(db, text, [
            "YOU ARE CARRYING SOME KEYS",
        ])).toContain("LAMP");
        expect(listVisibleRoomObjectsNotCarried(db, text, [], ["LAMP"])).not.toContain("LAMP");
    });
    ...
    it("deprioritize moves tokens to the tail of the motion slice", () => {
        ...
        const idxLook = (arr) => arr.indexOf("LOOK");
        const idxRoad = (arr) => arr.indexOf("ROAD");
...
describe("recentTextSuggestsVerticalPassageNavigation", () => {
    it("matches grate, pit, stair, ladder, and chasm cues", () => {
        expect(recentTextSuggestsVerticalPassageNavigation("YOU'RE OUTSIDE GRATE. THE STREAM BED.\n")).toBe(true);
        expect(recentTextSuggestsVerticalPassageNavigation("A METAL GRATE IN THE TWENTY FOOT DEPRESSION.\n")).toBe(true);
        expect(recentTextSuggestsVerticalPassageNavigation("YOU ARE AT THE BRINK OF A DEEP PIT.\n")).toBe(true);
        expect(recentTextSuggestsVerticalPassageNavigation("ROUGH STONE STEPS LEAD DOWN.\n")).toBe(true);
        expect(recentTextSuggestsVerticalPassageNavigation("THERE IS A RICKETY WOODEN LADDER HERE.\n")).toBe(true);
        expect(recentTextSuggestsVerticalPassageNavigation("YOU ARE ON THE EDGE OF A BREATHTAKING CHASM.\n")).toBe(true);
    });
    it("is false without vertical-passage cues", () => {
        expect(recentTextSuggestsVerticalPassageNavigation("YOU ARE IN A FOREST.\n")).toBe(false);
        expect(recentTextSuggestsVerticalPassageNavigation("ABOUT A GRATE OF CARROTS.\n")).toBe(false);
        expect(recentTextSuggestsVerticalPassageNavigation("A DEEP FOREST PITTED WITH STUMPS.\n")).toBe(false);
    });
    it("grateDescentNavigation alias matches vertical passage", () => {
        const s = "YOU'RE OUTSIDE GRATE.\n";
        expect(recentTextSuggestsGrateDescentNavigation(s)).toBe(recentTextSuggestsVerticalPassageNavigation(s));
    });
});
describe("recentTextSuggestsIndoorBuildingNavigation", () => {
    it("is true for well house room line", () => {
        expect(recentTextSuggestsIndoorBuildingNavigation("YOU ARE INSIDE A BUILDING, A WELL HOUSE FOR A LARGE SPRING.\n")).toBe(true);
    });
    it("is true when compass failed with YOU'RE INSIDE BUILDING", () => {
        expect(recentTextSuggestsIndoorBuildingNavigation("THERE IS NO WAY TO GO THAT DIRECTION. YOU'RE INSIDE BUILDING. LAMP.\n")).toBe(true);
    });
    it("is false for standing outside near a building", () => {
        expect(recentTextSuggestsIndoorBuildingNavigation("YOU ARE STANDING AT THE END OF A ROAD BEFORE A SMALL BRICK BUILDING.\n")).toBe(false);
    });
});
describe("formatSituationalCandidatesSection", () => {
    it("appends optional appendix and uses token — hint lines", () => {
        const db = loadDatFile(datPath);
        const s = formatSituationalCandidatesSection(db, ["EAST", "WEST"], "**Try next:** NORTH");
    ...
    it("flatList emits comma-separated tokens only", () => {
        const db = loadDatFile(datPath);
        const s = formatSituationalCandidatesSection(db, ["EAST", "WEST", "TAKE"], undefined, { flatList: true });
        expect(s).toContain("EAST, WEST, TAKE");
        expect(s).not.toMatch(/\*\*EAST\*\*\s*—/);
    });
    it("slmGrouped splits motion vs action vs object", () => {
        const db = loadDatFile(datPath);
        const s = formatSituationalCandidatesSection(db, ["EAST", "WEST", "TAKE", "KEYS"], undefined, { slmGrouped: true });
    ...
    it("lootFunnelDeferCandMove explains deferred travel when move list empty", () => {
        const db = loadDatFile(datPath);
        const s = formatSituationalCandidatesSection(db, ["TAKE", "GET", "KEYS"], undefined, { slmGrouped: true, lootFunnelDeferCandMove: true });
    ...
    it("groups pickup before travel when room objects are in the list", () => {
        const db = loadDatFile(datPath);
        const text = "YOU'RE INSIDE BUILDING. THERE ARE SOME KEYS ON THE GROUND. THERE IS A SHINY BRASS LAMP NEARBY.";
...
describe("parseRelevantTokensResponse", () => {
    it("filters to allowed set and uppercases", () => {
        ...
        expect(parseRelevantTokensResponse({ relevantTokens: ["east", "NORTH", "TAKE"] }, allowed)).toEqual(["EAST", "TAKE"]);
    });
});
//# sourceMappingURL=situationalCandidates.test.js.map