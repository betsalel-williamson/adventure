...
import { adventureLlmAutoplayJsonFooter, buildAutoplayPlannerPrompt, buildInterpretSystemAndUserPrompt, linesForAutoplayPlannerContextBody, mlxAutoplaySystemPrompt, mlxAutoplaySystemPromptForVariant, resolveCompactPrompts, resolveFastInterpretPrompt, resolveInterpretPromptBuildOptions, resolveInterpretPromptExamples, recentGameTextSliceForInterpretPrompt, resolveMlxSystemPromptVariant, resolveAutoplayPromptMode, resolveStructuredDashboardPrompts, resolveVocabHintMaxWords, } from "@adventure-llm/nl-glue";
...
describe("MLX system prompt variants (ADVENTURE_LLM_MLX_SYSTEM_VARIANT)", () => {
    ...
    it("mlxAutoplaySystemPrompt uses env variant", () => {
        ...
        delete process.env.ADVENTURE_LLM_MLX_SYSTEM_VARIANT;
        expect(mlxAutoplaySystemPrompt(true)).toBe(mlxAutoplaySystemPromptForVariant(true, "full"));
    });
...
describe("compact NL prompts", () => {
    ...
    beforeEach(() => {
        process.env.ADVENTURE_LLM_VOCAB_CATEGORIES_FILE = path.join(tmpdir(), `nl-prompts-no-ai-vocab-${Date.now()}.json`);
    ...
    it("compact interpret prompt uses HELP excerpt + cue, not full HELP preamble; shorter than full", () => {
        ...
        const compact = buildInterpretSystemAndUserPrompt(db, "go east", "ROOM TEXT", {
            compact: true,
            providerId: "google",
        });
    ...
    it("split planner body merges without appending global JSON footer (system already defines keys)", () => {
        ...
        const merged = buildAutoplayPlannerPrompt(db, {
...
describe("resolveInterpretPromptBuildOptions", () => {
    ...
    it("uses overrides when set", () => {
        expect(resolveInterpretPromptBuildOptions("google", {
            compact: true,
            structuredDashboard: true,
...
describe("resolveInterpretPromptExamples (ADVENTURE_LLM_INTERPRET_PROMPT_EXAMPLES)", () => {
    const prev = { ...process.env };
    beforeEach(() => {
        process.env.ADVENTURE_LLM_VOCAB_CATEGORIES_FILE = path.join(tmpdir(), `examples-no-ai-vocab-${Date.now()}.json`);
        delete process.env.ADVENTURE_LLM_VOCAB_AI_CATEGORIES;
...
//# sourceMappingURL=adventureNlPrompts.test.js.map