 ...
 * Requires OpenSSL 1.1.1+ on PATH. Output: adventure-nl/.cache/tls/dev-{key,cert}.pem
 */
...
if (existsSync(keyOut) && existsSync(certOut)) {
  process.stderr.write(
    `adventure-nl: TLS files already exist:\n  ${keyOut}\n  ${certOut}\n`,
  );
...
if (r.status !== 0) {
  process.stderr.write(
    "openssl failed. Install OpenSSL 1.1.1+ or use ADVENTURE_NL_WEB_INSECURE_HTTP=1.\n",
  );
  process.exit(r.status ?? 1);
}

process.stderr.write(
  `adventure-nl: wrote dev TLS files:\n  ${keyOut}\n  ${certOut}\n`,
);
process.stderr.write(browserSelfSignedHint);
