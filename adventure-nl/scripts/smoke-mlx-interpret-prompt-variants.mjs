 ...
 * Clears ADVENTURE_NL_CACHE_DIR for the process so prompt shape does not hit stale cache.
 */
...
delete process.env.ADVENTURE_NL_CACHE_DIR;
...
const { loadInterpretEvalFixtures } = await import("@adventure-nl/nl-glue");
const { compareInterpretEval } = await import("@adventure-nl/nl-glue");
...
async function main() {
  ...
  if (!client || client.providerId !== "mlx") {
    console.error(
      "smoke-mlx-interpret-prompt-variants: set ADVENTURE_NL_TEXT_PROVIDER=mlx and a working MLX setup (uv sync).",
    );
    process.exitCode = 1;
    return;
  }
  ...
  for (const variant of PROMPT_VARIANTS) {
    process.env.ADVENTURE_NL_INTERPRET_PROMPT_EXAMPLES = variant.promptExamples
      ? "1"
      : "0";
    let pass = 0;
    let fail = 0;
