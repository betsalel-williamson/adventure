#!/usr/bin/env node
/**
 * One MLX worker session: run the autoplay planner once per ADVENTURE_NL_MLX_SYSTEM_VARIANT value.
 * Avoids restarting `npm start` for each variant (single model load).
 *
 * Usage (from adventure-nl, after `npm run build`):
 *   node scripts/smoke-mlx-system-variants.mjs
 *
 * Requires: Apple Silicon MLX, `uv sync`, adventure.dat at repo root (parent of adventure-nl).
 */
...
process.env.ADVENTURE_NL_TEXT_PROVIDER ??= "mlx";
process.env.ADVENTURE_NL_AUTOPLAY_TWO_STEP = "0";
process.env.ADVENTURE_NL_DEBUG = process.env.ADVENTURE_NL_DEBUG ?? "0";
...
} = await import("@adventure-nl/nl-glue");
...
const contextChars = Number(
  process.env.ADVENTURE_NL_AUTOPLAY_CONTEXT_CHARS ?? "6000",
);
...
async function main() {
  ...
  if (!client || client.providerId !== "mlx") {
    console.error(
      "smoke-mlx-system-variants: need ADVENTURE_NL_TEXT_PROVIDER=mlx and a working MLX setup (uv sync).",
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (!structuredDashboard) {
    console.error(
      "smoke-mlx-system-variants: set ADVENTURE_NL_STRUCTURED_PROMPTS=1 for MLX structured prompts (default).",
    );
    process.exitCode = 1;
    return;
  }
  ...
  for (const variant of MLX_SYSTEM_PROMPT_VARIANTS) {
    process.env.ADVENTURE_NL_MLX_SYSTEM_VARIANT = variant;
