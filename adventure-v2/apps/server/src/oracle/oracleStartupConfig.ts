...
const defaultBridgeScript = join(adventureV2Root, "fixtures", "oracle-fortran-bridge.mjs");
...
export type OracleStartupResolution =
  ...
  | { kind: "process"; scriptPath: string; reason: "explicit_env" | "auto_fortran" };
...
export function resolveOracleStartupConfig(): OracleStartupResolution {
  ...
  if (explicitRaw !== undefined) {
    ...
    if (explicit.length > 0) {
      return { kind: "process", scriptPath: resolve(explicit), reason: "explicit_env" };
    }
  ...
  if (existsSync(defaultBridgeScript) && existsSync(defaultAdventureBinary)) {
    return { kind: "process", scriptPath: defaultBridgeScript, reason: "auto_fortran" };
  }
