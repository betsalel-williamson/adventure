...
WHILE you play with the map panel enabled THE map SHALL update from visible transcript lines without blocking your command input.

WHEN assist or agent services are unreachable THEN you may see a stale diagram, a locally merged draft, or a status message — keep playing from the CRT transcript when the game API is healthy.

## Honest notes

WHEN you record research results THEN label map output as **inferred from visible transcript**, not as ground-truth cave topology.

Shared terms: [Exploration map](../../glossary/exploration-map.md) · [Draft assistance](../../glossary/draft-assistance.md)

Related: [Exploration map guide (langgraph)](../exploration-map-guide.md) · [Features — exploration map Mermaid](../../features/webclient/exploration-map-mermaid.md)
