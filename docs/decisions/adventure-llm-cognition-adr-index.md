...
5. [ADR0008](ADR0008-server-subsystem-replica-and-sync.md) — Server subsystem replica and sync on connect (**accepted**; **`POST /api/subsystem-sync`** + `subsystemServerSync.ts` — see ADR **Implementation**; browser orchestration / restore-from-server forward work)
