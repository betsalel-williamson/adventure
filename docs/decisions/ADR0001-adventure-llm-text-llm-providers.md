...
2. **Interpret cache keys** — Disk cache entries use `interpretCacheKeyFromBuildOptions`, which hashes schema version (`interpretCacheSchemaVersion` / `ADVENTURE_LLM_CACHE_SCHEMA_VERSION`), provider, model, user text, compact/structured layout flags, and the same recent-game tail slice as `recentGameTextSliceForInterpretPrompt`.
3. **Interactive NL session context** — By default, the CLI prepends `AutoplaySessionMemory.buildInteractiveInterpretPrefix` to recent game text so human play sees heuristic state, turn summaries, and situational candidate tokens (`ADVENTURE_LLM_INTERACTIVE_SESSION=0` restores the prior thin behavior).
4. **Latency toggle** — `ADVENTURE_LLM_FAST_INTERPRET=1` forces interpret few-shot examples off regardless of provider defaults.
5. **HTTP transport** — Failures use `LlmTransportError` with optional `status`. Transient statuses (429, 502, 503, 500) may retry up to `ADVENTURE_LLM_HTTP_RETRY_ATTEMPTS` (1–5).
6. **Debug logs** — JSONL records pass through `sanitizeInteractionLogRecord`, truncating long `prompt` / `system` / `user` / `rawJson` fields per `ADVENTURE_LLM_DEBUG_MAX_PROMPT_CHARS`.
...
- [adventure-llm/src/nl/interpretDiskCache.ts](../../adventure-llm/src/nl/interpretDiskCache.ts)
- [adventure-llm/src/nl/llmDebug.ts](../../adventure-llm/src/nl/llmDebug.ts) — cache schema version, JSONL sanitization.
- [adventure-llm/src/nl/llmErrors.ts](../../adventure-llm/src/nl/llmErrors.ts) — `LlmTransportError`, fallback classification.
- [adventure-llm/src/nl/autoplaySessionMemory.ts](../../adventure-llm/src/nl/autoplaySessionMemory.ts) — autoplay and interactive session prefix.
- [adventure-llm/src/cli/main.ts](../../adventure-llm/src/cli/main.ts)
