...
2. **Interpret cache keys** — Disk cache entries use `interpretCacheKeyFromBuildOptions`, which hashes schema version (`interpretCacheSchemaVersion` / `ADVENTURE_LM_CACHE_SCHEMA_VERSION`), provider, model, user text, compact/structured layout flags, and the same recent-game tail slice as `recentGameTextSliceForInterpretPrompt`.
3. **Interactive NL session context** — By default, the CLI prepends `AutoplaySessionMemory.buildInteractiveInterpretPrefix` to recent game text so human play sees heuristic state, turn summaries, and situational candidate tokens (`ADVENTURE_LM_INTERACTIVE_SESSION=0` restores the prior thin behavior).
4. **Latency toggle** — `ADVENTURE_LM_FAST_INTERPRET=1` forces interpret few-shot examples off regardless of provider defaults.
5. **HTTP transport** — Failures use `LlmTransportError` with optional `status`. Transient statuses (429, 502, 503, 500) may retry up to `ADVENTURE_LM_HTTP_RETRY_ATTEMPTS` (1–5).
6. **Debug logs** — JSONL records pass through `sanitizeInteractionLogRecord`, truncating long `prompt` / `system` / `user` / `rawJson` fields per `ADVENTURE_LM_DEBUG_MAX_PROMPT_CHARS`.
...
- [guidelines/adventure-lm/autoplay-planner-context.md](../../guidelines/adventure-lm/autoplay-planner-context.md) — autoplay prompt modes (`explore` / `full`), candidate hygiene, object scope, guards, web pace overrides.
- [adventure-lm/src/nl/textLlmContract.ts](../../adventure-lm/src/nl/textLlmContract.ts)
- [adventure-lm/src/nl/textLlmInterpretPipeline.ts](../../adventure-lm/src/nl/textLlmInterpretPipeline.ts)
- [adventure-lm/src/nl/interpretCacheKey.ts](../../adventure-lm/src/nl/interpretCacheKey.ts)
- [adventure-lm/src/nl/interpretDiskCache.ts](../../adventure-lm/src/nl/interpretDiskCache.ts)
- [adventure-lm/src/nl/llmDebug.ts](../../adventure-lm/src/nl/llmDebug.ts) — cache schema version, JSONL sanitization.
- [adventure-lm/src/nl/llmErrors.ts](../../adventure-lm/src/nl/llmErrors.ts) — `LlmTransportError`, fallback classification.
- [adventure-lm/src/nl/autoplaySessionMemory.ts](../../adventure-lm/src/nl/autoplaySessionMemory.ts) — autoplay and interactive session prefix.
- [adventure-lm/src/cli/main.ts](../../adventure-lm/src/cli/main.ts)
