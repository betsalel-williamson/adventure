...
The client SQLite database is **authoritative** ([ADR0006](ADR0006-client-sqlite-wal-subsystem-store.md) — store module and migrations in `adventure-nl/src/browser/`). **Tag rows** (`revision_tags`, [ADR0007](ADR0007-subsystem-revision-control-and-replay.md)) are part of the same client-originated state and should replicate with subsystem revisions in sync batches. The server maintains a **replica** (e.g. `better-sqlite3` under `adventure-nl`, similar to `benchmarkRunsDb.ts`).
...
| Area | Path |
| ---- | ---- |
| Sync batch apply (linear push, fork detection, idempotent retries, behind-server noop) + HTTP body parser + workspace id validation | [`adventure-nl/src/cli/subsystemServerSync.ts`](../../adventure-nl/src/cli/subsystemServerSync.ts) |
| Vitest (apply, fork, behind, tags) | [`adventure-nl/src/cli/subsystemServerSync.test.ts`](../../adventure-nl/src/cli/subsystemServerSync.test.ts) |
| **`POST /api/subsystem-sync`** route (session cookie; same `/api` session resolution as other dashboard JSON routes) | [`adventure-nl/src/cli/webDashboard.ts`](../../adventure-nl/src/cli/webDashboard.ts) |
| HTTP integration test | [`adventure-nl/src/cli/webDashboard.test.ts`](../../adventure-nl/src/cli/webDashboard.test.ts) |
| Client `fetch` helper | [`adventure-nl/public/dashboardApi.js`](../../adventure-nl/public/dashboardApi.js) (`postSubsystemSync`) |

Replica databases use the **same migrations** as the client store ([`subsystemWalStoreMigrations.ts`](../../adventure-nl/src/browser/subsystemWalStoreMigrations.ts)) via `SubsystemWalStore` / `migrateSubsystemWalStore`. Default directory: **`adventure-nl/.cache/subsystem-replica/<workspaceId>.db`**. Override with env **`ADVENTURE_NL_SUBSYSTEM_REPLICA_DIR`** (absolute or resolved path for the **directory** containing `<workspaceId>.db` files).
...
- `adventure-nl/src/cli/benchmarkRunsDb.ts`
- `adventure-nl/src/cli/webDashboardSession.ts`
