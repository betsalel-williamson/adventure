...
**A. Already in `@adventure-nl/nl-glue` (move is done for these; keep tests and exports in sync):**
...
**B. Phase A — still under `adventure-nl/src/nl/` (glue; target move into `@adventure-nl/nl-glue` before Phase B):**
...
| Category | Files |
| -------- | ----- |
| **Map / exploration / viz** | [`inferredExplorationMap.ts`](../../adventure-nl/src/nl/inferredExplorationMap.ts), [`explorationGraphViz.ts`](../../adventure-nl/src/nl/explorationGraphViz.ts) |
| **Intent / vocab coercion** | [`intent.ts`](../../adventure-nl/src/nl/intent.ts), [`coerceToVocab.ts`](../../adventure-nl/src/nl/coerceToVocab.ts) |
| **Session memory + planner path** | [`autoplaySessionMemory.ts`](../../adventure-nl/src/nl/autoplaySessionMemory.ts), [`autoplayPlannerGuards.ts`](../../adventure-nl/src/nl/autoplayPlannerGuards.ts), [`plannerToScriptedGetin.ts`](../../adventure-nl/src/nl/plannerToScriptedGetin.ts), [`buildAutoplayPlannerInvocation.ts`](../../adventure-nl/src/nl/buildAutoplayPlannerInvocation.ts) |
| **Prompts + JSON shaping** | [`adventureNlPrompts.ts`](../../adventure-nl/src/nl/adventureNlPrompts.ts), [`jsonFromLlmText.ts`](../../adventure-nl/src/nl/jsonFromLlmText.ts), [`coerceLlmJson.ts`](../../adventure-nl/src/nl/coerceLlmJson.ts), [`promptExperiment.ts`](../../adventure-nl/src/nl/promptExperiment.ts) |
| **Interpret pipeline glue** | [`textLlmInterpretPipeline.ts`](../../adventure-nl/packages/nl-glue/src/textLlmInterpretPipeline.ts) |
| **Shared types** | [`textLlmContract.ts`](../../adventure-nl/src/nl/textLlmContract.ts) (or fold into package `schema` / exports after review) |
| **Fixtures / codegen helpers** | [`interpretEvalFixtures.ts`](../../adventure-nl/src/nl/interpretEvalFixtures.ts), [`vocabCategoriesGenerate.ts`](../../adventure-nl/src/nl/vocabCategoriesGenerate.ts) — may stay in `adventure-nl` if only scripts/tests need them; otherwise co-locate with glue tests |

**C. Shim / pipe (stay in `adventure-nl` until Phase B; thin adapters, not duplicate glue):**

| Location | Role |
| -------- | ---- |
| [`src/cognition/dashboardServerLlmReactions.ts`](../../adventure-nl/src/cognition/dashboardServerLlmReactions.ts) | Server-side “reactions” to LM output; should **call** `@adventure-nl/nl-glue` (and moved modules from B), not reimplement them |
...
| [`src/browser/shims/`](../../adventure-nl/src/browser/shims/) | Browser bundle shims (e.g. `vocabAiCategories.browser.ts`) until Phase B consumes the package directly |
