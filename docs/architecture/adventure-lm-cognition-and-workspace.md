...
**Related decisions:** [ADR0004](../decisions/ADR0004-backend-llm-packaging-and-discovery.md) through [ADR0014](../decisions/ADR0014-two-step-lm-glue-package-then-browser.md), indexed under [adventure-lm-cognition-adr-index.md](../decisions/adventure-lm-cognition-adr-index.md).
...
Decisions are recorded in ADRs (not duplicated here). Dependency order for implementation: see [adventure-lm-cognition-adr-index.md](../decisions/adventure-lm-cognition-adr-index.md).
...
The **target** integration point for the dashboard path is a **single orchestration hub**—implemented with **XState v5** (`adventure-lm/src/browser/autoplayCognitionMachine.ts`, bundled for the browser) and **converging** with the imperative client loop in `public/browserAutoplayOrchestrator.js`—that sequences:
...
**CLI:** May retain the **legacy** full pipeline in Node ([`cli/main.ts`](../../adventure-lm/src/cli/main.ts)) until explicitly aligned with the shared client library.

## Deployment view

- **Single-machine / local dev:** Node serves static `adventure-lm/public/`, Fortran binary beside `adventure.dat`, env-based LLM keys; browser holds SQLite WASM + OPFS (or chosen persistence).
...
- **Replica:** Server SQLite for backup and inspection ([ADR0008](../decisions/ADR0008-server-subsystem-replica-and-sync.md)); default path pattern **`adventure-lm/.cache/subsystem-replica/<workspaceId>.db`** (optional env **`ADVENTURE_LM_SUBSYSTEM_REPLICA_DIR`**).
...
- [adventure-lm-cognition-adr-index.md](../decisions/adventure-lm-cognition-adr-index.md) — ADR list and implementation order.
