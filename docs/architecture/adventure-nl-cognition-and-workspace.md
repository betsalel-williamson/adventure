...
Decisions are recorded in ADRs (not duplicated here). Dependency order for implementation: see [adventure-nl-cognition-adr-index.md](../decisions/adventure-nl-cognition-adr-index.md).
...
**CLI:** May retain the **legacy** full pipeline in Node ([`cli/main.ts`](../../adventure-nl/src/cli/main.ts)) until explicitly aligned with the shared client library.
