...
This document is limited to **`adventure.f`** and **`adventure.dat`**. The [`adventure-nl`](../../adventure-nl/) package loads the same dat file, spawns the built **`./adventure`** binary for GETIN-compatible play, and optionally maps natural language to the two five-letter columns described under **Vocabulary lookup** above. For that wrapper’s **current** structure (LLM providers, caching, CLI), see [adventure-engine.md](./adventure-engine.md) and [ADR0001: adventure-nl TextLlm providers](../decisions/ADR0001-adventure-nl-text-llm-providers.md). For the **target** direction of the web dashboard and cognition layers (browser policy, thin backend), see [adventure-nl-cognition-and-workspace.md](./adventure-nl-cognition-and-workspace.md) and [adventure-nl-cognition-adr-index.md](../decisions/adventure-nl-cognition-adr-index.md).
