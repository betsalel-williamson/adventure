...
This document is limited to **`adventure.f`** and **`adventure.dat`**. The [`adventure-lm`](../../adventure-lm/) package loads the same dat file, spawns the built **`./adventure`** binary for GETIN-compatible play, and optionally maps natural language to the two five-letter columns described under **Vocabulary lookup** above. For that wrapper’s **current** structure (LLM providers, caching, CLI), see [adventure-engine.md](./adventure-engine.md) and [ADR0001: adventure-lm TextLlm providers](../decisions/ADR0001-adventure-lm-text-llm-providers.md). For the **target** direction of the web dashboard and cognition layers (browser policy, thin backend), see [adventure-lm-cognition-and-workspace.md](./adventure-lm-cognition-and-workspace.md) and [adventure-lm-cognition-adr-index.md](../decisions/adventure-lm-cognition-adr-index.md).
