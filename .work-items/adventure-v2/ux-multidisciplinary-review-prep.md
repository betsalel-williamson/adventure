...
| **U3** | UX copy: oracle mode + stub autoplay | Code | U2 (persona / messaging) | [`index.html`](../../adventure-v2/apps/web/index.html) intro leads with **which oracle** (synthetic vs process vs Fortran) and states stub autoplay is **not** LLM; optional banner text when API URL unreachable pattern exists. |
| **U4** | Default **raw SSE** off for first-time shell | Code | U3 optional | Checkbox default **unchecked** *or* persist last toggle in [`shellSessionPersistence`](../../adventure-v2/apps/web/src/shellSessionPersistence.ts); README one-line if behavior changes. |
| **U5** | Empty / loading states for game lane | Code | U2 | Clear placeholder until first `oracle_observation` append (ties to P0 gap); Vitest on pure formatters if logic grows ([`wireDisplay.ts`](../../adventure-v2/apps/web/src/wireDisplay.ts)). |
