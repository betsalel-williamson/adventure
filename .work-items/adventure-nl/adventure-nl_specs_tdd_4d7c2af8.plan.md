---
name: adventure-nl specs TDD
overview: Decompose Colossal Cave Adventure into documented contracts and a TypeScript test pyramid (unit → integration → E2E), then implement a new engine that loads unchanged `adventure.dat` and an `adventure-nl` layer that maps natural language to game tokens via Gemini structured output, with optional imagery—preserving the classic text-adventure feel.
todos:
  - id: work-items-docs
    content: Add `.work-items/adventure-nl/` user-story.md, design.md (dat grammar, parity, Gemini schema, fallbacks), bugs.md (known Fortran behaviors to replicate), task.md + numbered steps; add `docs/architecture/adventure-engine.md`
    status: pending
  - id: ts-scaffold
    content: Create `adventure-nl/` TypeScript package (strict TS, Vitest, npm scripts, README env vars)
    status: pending
  - id: tdd-dat-loader
...
# adventure-nl: specs, TDD harness, and Gemini-enhanced UX
...
Create a dedicated work item folder per project conventions, e.g. `[.work-items/adventure-nl/](.work-items/adventure-nl/)`, containing:
...
Introduce a **TypeScript** package (your choice) at repo root, e.g. `[adventure-nl/](adventure-nl/)` with `package.json`, TypeScript strict, Vitest (or Jest), and a single `npm test` / `npm run check` preflight.
...
Suggested module boundaries inside `adventure-nl/`:
...
## Phase 4 — `adventure-nl` natural language layer (Gemini structured output)
...
- Add `adventure-nl/` TypeScript project + CI job running `npm test` and typecheck.
- Link README sections: how to run classic `./adventure`, how to run `adventure-nl` CLI, env vars for `GEMINI_API_KEY`.
...
- Written specs in `[.work-items/adventure-nl/](.work-items/adventure-nl/)` + `[docs/architecture/adventure-engine.md](docs/architecture/adventure-engine.md)`, including **documented known behaviors** (`bugs.md` or equivalent) aligned with Fortran oracle tests.
...
- Clear **out-of-scope** line: “corrected / extended” gameplay rules are not part of `adventure-nl` unless/until a separate initiative explicitly defines a new baseline.
