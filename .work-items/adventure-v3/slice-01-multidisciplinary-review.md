...
Audience: UX, architecture, operations. Scope: [`adventure-v3`](../../adventure-v3/) thin client + [`adventure-v2`](../../adventure-v2/) HTTP oracle wiring consumed by that client.
...
| --- | --- | --- |
| P0 | Users cannot tell if Fortran is active without reading logs | Status strip uses plain-language copy from `GET /health` ([`describeHealthStatus`](../../adventure-v3/apps/web/src/status/health.ts)). |
| P1 | Command field should be easy to reach after load | `focus()` on `#command-input` after bootstrap ([`main.ts`](../../adventure-v3/apps/web/src/main.ts)). |
| P1 | Transcript updates should be announced when using assistive tech | `aria-live="polite"` on `#crt-transcript` ([`index.html`](../../adventure-v3/apps/web/index.html)). |
| P1 | Transcript should behave like a terminal (scroll tail-follow vs reading history) | Flex-filled CRT viewport + pinned scroll: follow tail only when near bottom or after **Send** ([`crt-shell.css`](../../adventure-v3/apps/web/styles/crt-shell.css), [`main.ts`](../../adventure-v3/apps/web/src/main.ts)). |
| P2 | Hero transcript feels empty before first room text | Silent bootstrap (`POST /turns` with empty `input` after SSE opens) declines instructions without painting an extra `look` ([`main.ts`](../../adventure-v3/apps/web/src/main.ts)); cognition **proposal** lines hidden from CRT (`appendFromWire`). |

## Architecture

| Priority | Finding | Resolution |
| --- | --- | --- |
| P0 | Cucumber must mirror CLI oracle resolution — synthetic vs process oracle must match `GET /health` | Hooks branch like [`cli.ts`](../../adventure-v2/apps/server/src/cli.ts): `resolveOracleStartupConfig()` → **`bridge_script`** uses [`createProcessOracleBridge`](../../adventure-v2/apps/server/src/oracle/processOracleBridge.ts); **`persistent_fortran`** uses [`createPersistentFortranOracleBridge`](../../adventure-v2/apps/server/src/oracle/persistentFortranOracleBridge.ts); else [`createSyntheticOracleBridge`](../../adventure-v2/apps/server/src/oracle/oracleBridge.ts) ([`http_hooks.ts`](../../adventure-v3/tests/cucumber/http_hooks.ts)). |
...
| Priority | Finding | Resolution |
| --- | --- | --- |
| P1 | Operators need a predictable test matrix | [`README.md`](../../adventure-v3/README.md) documents API dependency, ports (**5174** default for v3 `npm start` vs **5173** for v2-only web), and Fortran Cucumber script. |
...
- `cd adventure-v3 && npm run test:cucumber:fortran` (optional, with `./adventure`)
