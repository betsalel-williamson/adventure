...
- **Location agent:** Optional assist HTTP call (or in-process merge) that turns **new transcript line(s) + bounded context** into a **schema-valid graph patch**; on failure or when `locationAgent` is off, **`mergeGraphFromTranscript`** in [`map-core`](../../adventure-v3/packages/map-core/src/kernel.ts) is the deterministic floor.
- **Graph reducer:** Applies patches to [`DirectedMapGraph`](../../adventure-v3/packages/map-core/src/directedGraph.ts); serializes to Mermaid via [`directedGraphToMermaidFlowchart`](../../adventure-v3/packages/map-core/src/mermaid.ts).
...
**Feature flags:** Central [`featureFlags`](../../adventure-v3/apps/web/src/featureFlags.ts) module; build defaults via `VITE_V3_*`; optional dev overrides documented in [`adventure-v3/README.md`](../../adventure-v3/README.md). Assist server honors `ASSIST_PROBE_ENABLED` for automated probe steps.
